...
export default function GlobalMeta() {
  ...
  useEffect(() => {
    const nomePortal  = cfg.nome_site         || 'Portal de notícias'
    const titulo      = cfg.site_titulo       || nomePortal
    ...
    const author      = cfg.site_author       || nomePortal
    ...
    // ── Canonical + dados estruturados do portal ─────────────
    let canonical = document.querySelector('link[rel="canonical"]')
    if (!canonical) { canonical = document.createElement('link'); canonical.rel = 'canonical'; document.head.appendChild(canonical) }
    canonical.href = cfg.site_url || window.location.origin
    setMeta('og:locale', 'pt_BR', true)

    let websiteLd = document.getElementById('website-jsonld')
    if (!websiteLd) { websiteLd = document.createElement('script'); websiteLd.id = 'website-jsonld'; websiteLd.type = 'application/ld+json'; document.head.appendChild(websiteLd) }
    websiteLd.textContent = JSON.stringify({
      '@context':'https://schema.org',
      '@graph':[
        {'@type':'NewsMediaOrganization','@id':`${cfg.site_url||window.location.origin}/#organization`,name:nomePortal,url:cfg.site_url||window.location.origin,description:descricao,logo:favicon||imagem||undefined},
        {'@type':'WebSite','@id':`${cfg.site_url||window.location.origin}/#website`,name:nomePortal,url:cfg.site_url||window.location.origin,publisher:{'@id':`${cfg.site_url||window.location.origin}/#organization`},potentialAction:{'@type':'SearchAction',target:`${cfg.site_url||window.location.origin}/?busca={search_term_string}`,'query-input':'required name=search_term_string'}},
      ],
    })
