...
export default function AdminAtualizacoes(){
  ...
  const [githubPublish,setGithubPublish]=useState(null),[ephemeralStage,setEphemeralStage]=useState(null),[engineTest,setEngineTest]=useState(null),[diagnostics,setDiagnostics]=useState(null),[systemTest,setSystemTest]=useState(null)
  ...
  return <div className="adm-page" style={{display:'grid',gap:16}}>
    ...
    {!serverless&&<div style={card}>
      <div style={{display:'flex',justifyContent:'space-between',gap:14,alignItems:'center',flexWrap:'wrap'}}>
        <div><h2 style={{margin:'0 0 5px',color:C.text,fontSize:18}}>Autoteste pós-instalação</h2><p style={{color:C.muted,fontSize:13,margin:0,lineHeight:1.55}}>Confere a instalação atual sem alterar dados: backend, MongoDB, versões, arquivos essenciais, gravação, health check, RSS, portal e integrações conectadas.</p></div>
        <button onClick={runSystemSelfTest} disabled={systemTest?.loading||Boolean(activeOperation)} style={{...btn,background:C.greenSolid,color:'#fff',opacity:(systemTest?.loading||Boolean(activeOperation)) ? .55 : 1}}>{systemTest?.loading?'Executando testes…':'Executar autoteste completo'}</button>
      </div>
      {systemTest&&!systemTest.loading&&<div style={{marginTop:14,padding:12,borderRadius:12,background:'var(--adm-surface2)',border:`1px solid ${systemTest.ok?C.greenSolid:C.red}`}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><div><strong style={{color:systemTest.ok?C.greenSolid:C.red}}>{systemTest.ok?'✓ Instalação saudável':'✕ Instalação precisa de atenção'}</strong><div style={{fontSize:12,color:C.muted,marginTop:3}}>Pontuação {systemTest.score??'—'}% · {systemTest.summary?.passed||0} aprovados · {systemTest.summary?.warnings||0} avisos · {systemTest.summary?.failed||0} falhas</div></div><button onClick={copySystemTest} style={{...btn,background:'var(--adm-surface)',color:C.text,border:'1px solid var(--adm-border)'}}>📋 Copiar diagnóstico</button></div>
        {!!systemTest.error&&<div style={{marginTop:10,color:C.red,fontSize:12}}>{systemTest.error}</div>}
        {!!systemTest.checks?.length&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8,marginTop:10}}>{systemTest.checks.map(c=><div key={c.id} style={{padding:10,borderRadius:9,border:'1px solid var(--adm-border)',background:'var(--adm-surface)',fontSize:12}}><div style={{fontWeight:800,color:c.status==='pass'?C.greenSolid:c.status==='skip'?C.muted:c.status==='warn'?'#f59e0b':C.red}}>{c.status==='pass'?'✓':c.status==='skip'?'○':c.status==='warn'?'⚠':'✕'} {c.label}</div><div className="updates-wrap" style={{marginTop:4,color:C.muted,lineHeight:1.45}}>{typeof c.detail==='string'?c.detail:JSON.stringify(c.detail)}</div>{c.durationMs>0&&<div style={{marginTop:4,color:C.muted,fontSize:11}}>{c.durationMs} ms</div>}</div>)}</div>}
      </div>}
    </div>}
