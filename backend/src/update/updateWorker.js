...
async function buildFrontend(){ const pkg=await readJson(path.join(ROOT_DIR,'frontend/package.json')); if(pkg.scripts?.build && process.env.AL_UPDATE_BUILD_FRONTEND!=='false') await run('npm',['run','build'],path.join(ROOT_DIR,'frontend'),{timeoutMs:8*60*1000}) }
...
try{
  ...
  try{
    ...
    await phase('build','Preparando frontend',64); await buildFrontend()
