...
router.post('/export', async(req,res,next)=>{ try {
  ...
  if(format==='json'){
    ...
    const body={product:'AL Sistemas',backupVersion:2,sourceVersion:'1.0.91',migrationCompatible:true,portableSecrets:includeSecrets,exportedAt:new Date().toISOString(),encoding:'UTF-8',includesSecrets:includeSecrets,accounts:identityStatus,variables:Object.fromEntries(rows.map(r=>[r.name,r.value]))}
