...
function PlatformNode({kind,title,subtitle,status,url,deploymentUrl,deploy,onDeploys,onConfigure,onVariables,onLogs,onPrimaryAction,primaryActionLabel}) {
  ...
  return <article className={`plat-node ${kind}`}>
    ...
    <p>{subtitle}</p>
    {url&&<div className="plat-url-block"><small>{isVercel?'DOMÍNIO DE PRODUÇÃO':'URL DA API'}</small><a className="plat-url" href={url} target="_blank" rel="noreferrer">{url.replace(/^https?:\/\//,'')} ↗</a></div>}
    {isVercel&&deploymentUrl&&deploymentUrl!==url&&<div className="plat-deployment-url"><small>DEPLOYMENT ATUAL</small><span>{deploymentUrl.replace(/^https?:\/\//,'')}</span><em>URL técnica do deploy; o portal público usa o domínio acima.</em></div>}
...
export default function AbaPlataformas(){
  ...
  async function refreshOrigins(){
    try{
      const r=await infraestruturaService.recarregarOrigensPlataformas()
      toast.success(r.message||'Domínio público e origens da Vercel sincronizados com o backend.')
      await load(true)
    }catch(e){toast.error(e.message)}
  }
  ...
  return <div className="platform-center">
    ...
    <section className="plat-grid">
      <PlatformNode
        ...
        status={vcDeploy?.estado} url={data.producao?.frontendOrigin||''} deploymentUrl={data.producao?.currentDeploymentUrl||vcDeploy?.url||''} deploy={vcDeploy}
