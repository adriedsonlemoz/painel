...
function AbaRecursos() {
  ...
  async function removeResource(type,item){
    ...
    if(!await confirmAction(`Excluir "${label}" da Cloudflare? Esta ação pode remover dados/configuração.`, { title:'Excluir recurso da Cloudflare', confirmLabel:'Excluir' }))return
    setBusy(true)
    try{
...
function AbaDns({ zona }) {
  ...
  async function purgeCache() {
    if (!await confirmAction(`Purgar TODO o cache de "${zona.name}"? Isso pode aumentar o tráfego temporariamente.`, { title:'Purgar cache', confirmLabel:'Purgar', variant:'warning' })) return
    setPurgando(true)
    try {
...
function AbaR2({ status, onRefreshStatus }) {
  ...
  async function excluirBucket(bucket,e){e?.stopPropagation?.();const u=usage?.buckets?.find(x=>(x.nome||x.name)===bucket);const count=Number(u?.objetos||0);const msg=count?`O bucket “${bucket}” possui ${count.toLocaleString('pt-BR')} objeto(s) e não pode ser excluído enquanto não estiver vazio.`:`Excluir definitivamente o bucket vazio “${bucket}”?`;if(count)return toast.error(msg);if(!await confirmAction(msg,{title:'Excluir bucket',confirmLabel:'Excluir'}))return;setBusy(true);try{await cloudflareService.deletarBucket(bucket);toast.success('Bucket removido');await carregarOverview()}catch(err){toast.error(err.message)}finally{setBusy(false)}}
  ...
  async function deletarUm(key){if(!await confirmAction(`Excluir “${relativeName(key)}”?`,{title:'Excluir arquivo',confirmLabel:'Excluir'}))return;try{await cloudflareService.deletarObjeto(bn(bucketSel),key);toast.success('Arquivo removido');await carregarObjetos(bucketSel,prefix,'');await carregarOverview()}catch(err){toast.error(err.message)}}
  async function deletarSelecionados(){const keys=[...selected];if(!keys.length||!await confirmAction(`Excluir definitivamente ${keys.length} arquivo(s) selecionado(s)?`,{title:'Excluir arquivos',confirmLabel:'Excluir'}))return;setBusy(true);try{const d=await cloudflareService.deletarObjetos(bn(bucketSel),keys);d.erros?.length?toast.error(`${d.deletados||0} removido(s), ${d.erros.length} falha(s)`):toast.success(`${keys.length} arquivo(s) removido(s)`);await carregarObjetos(bucketSel,prefix,'');await carregarOverview()}catch(err){toast.error(err.message)}finally{setBusy(false)}}
  ...
  async function revogarShare(id){if(!await confirmAction('Revogar este link imediatamente?',{title:'Revogar compartilhamento',confirmLabel:'Revogar'}))return;setShareBusy(true);try{await cloudflareService.revogarCompartilhamento(bn(bucketSel),id);const d=await cloudflareService.listarCompartilhamentos(bn(bucketSel),shareItem.key);setShareList(d.shares||[]);toast.success('Link revogado')}catch(err){toast.error(err.message)}finally{setShareBusy(false)}}

  async function togglePublic(enabled){if(!bucketSel)return;const verb=enabled?'ativar o acesso público r2.dev':'desativar o acesso público r2.dev';if(!await confirmAction(`Deseja ${verb} para “${bn(bucketSel)}”? ${enabled?'Qualquer pessoa que conheça a URL poderá ler objetos do bucket.':''}`,{title:enabled?'Ativar acesso público':'Desativar acesso público',confirmLabel:enabled?'Ativar':'Desativar',variant:'warning'}))return;setSettingsBusy(true);try{const d=await cloudflareService.definirAcessoPublicoR2(bn(bucketSel),enabled);setPublicAccess(d);toast.success(enabled?'r2.dev ativado':'r2.dev desativado');await carregarSettings(bn(bucketSel))}catch(err){toast.error(err.message)}finally{setSettingsBusy(false)}}
  ...
  async function removeCors(){if(!await confirmAction('Remover somente a regra CORS criada pelo AL Sistemas? As demais regras serão preservadas.',{title:'Remover regra CORS',confirmLabel:'Remover'}))return;setSettingsBusy(true);try{const remaining=(settingsData?.cors?.rules||[]).filter(rule=>rule.id!=='al-sistemas');if(remaining.length)await cloudflareService.salvarCorsBucket(bn(bucketSel),remaining);else await cloudflareService.removerCorsBucket(bn(bucketSel));toast.success('Regra CORS do AL removida.');await carregarSettings(bn(bucketSel))}catch(err){toast.error(err.message)}finally{setSettingsBusy(false)}}
  ...
  async function removeDomain(domain){if(!await confirmAction(`Remover o domínio “${domain}” deste bucket?`,{title:'Remover domínio',confirmLabel:'Remover'}))return;setSettingsBusy(true);try{await cloudflareService.removerDominioR2(bn(bucketSel),domain);toast.success('Domínio removido');await carregarSettings(bn(bucketSel))}catch(err){toast.error(err.message)}finally{setSettingsBusy(false)}}
