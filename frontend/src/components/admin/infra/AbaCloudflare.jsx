...
function CredentialSecretRow({ label, field, info }) {
  ...
  return (
    <div style={{ padding: `${SPACE.sm}px 0`, borderBottom: `1px solid ${C.border}`, minWidth: 0 }}>
        ...
        {info?.configured && info?.revealable !== false && (
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', flexShrink: 0 }}>
            <button type="button" onClick={reveal} disabled={busy} aria-label={`${value ? 'Ocultar' : 'Visualizar'} ${label}`} title={`${value ? 'Ocultar' : 'Visualizar'} ${label}`}
              style={{ border: `1px solid ${C.border}`, borderRadius: RADIUS.sm, padding: '5px 8px', background: C.surface2, color: C.text, fontSize: 12, cursor: 'pointer' }}>
      ...
      <code style={{ display: 'block', marginTop: 7, maxWidth: '100%', minWidth: 0, padding: '7px 8px', borderRadius: RADIUS.sm, background: C.surface2, color: value ? C.text : C.muted, fontSize: 12, whiteSpace: value ? 'pre-wrap' : 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', overflowWrap: 'anywhere', wordBreak: value ? 'break-all' : 'normal' }}>
        {value || info?.masked || '—'}
      </code>
    </div>
  )
        ...
        <PageCard>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:8}}>
            {status.capabilities.map(c=><div key={c.id} style={{padding:10,borderRadius:RADIUS.md,border:`1px solid ${C.border}`,background:C.surface2}}>
              ...
              <div style={{fontSize:12,color:C.muted,marginTop:5,lineHeight:1.4}}>{c.ok?`${c.count||0} recurso(s) detectado(s)`:c.error||c.description}</div>
            </div>)}
          </div>
        </PageCard>
      )}
          ...
          return <article key={id} style={{border:`1px solid ${C.border}`,borderRadius:RADIUS.lg,padding:13,background:C.surface2,minWidth:0}}>
            ...
            {r.ok?<div style={{marginTop:11}}>
              {(r.items||[]).slice(0,4).map((item,i)=><div key={item.id||item.uuid||item.name||item.title||item.queue_id||i} style={{display:'flex',justifyContent:'space-between',gap:8,alignItems:'center',padding:'7px 0',borderTop:`1px solid ${C.border}`}}>
                <div style={{minWidth:0}}><b style={{display:'block',fontSize:11,color:C.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{def.name(item)}</b><small style={{fontSize:12,color:C.muted}}>{id==='pages'?(item.production_branch||'main'):id==='d1'?`${bytes(item.file_size||0)}${item.num_tables!=null?` · ${item.num_tables} tabela(s)`:''}`:id==='vectorize'?`${item.config?.dimensions||'—'} dim · ${item.config?.metric||'—'}`:id==='queues'?`${item.producers_total_count||0} produtor(es) · ${item.consumers_total_count||0} consumidor(es)`:id==='ai-gateway'?(item.collect_logs?'logs ativos':'logs desativados'):(item.id||item.uuid||'')}</small></div>
                <div style={{display:'flex',gap:4,flexShrink:0}}>
                  {id==='pages'&&<button onClick={()=>openDeploys(item)} style={{fontSize:12,padding:'4px 6px'}}>Deploys</button>}
                  <button disabled={busy} onClick={()=>removeResource(id,item)} style={{fontSize:12,padding:'4px 6px',color:CF.err}}>Excluir</button>
                </div>
              </div>)}
              {(r.items||[]).length>4&&<div style={{fontSize:12,color:C.muted,marginTop:7}}>+ {(r.items||[]).length-4} outro(s)</div>}
              <button onClick={()=>{setForm({name:'',branch:'main',dimensions:768,metric:'cosine'});setModal({type:id})}} style={{marginTop:10,width:'100%',fontSize:12,padding:7}}>+ Criar {def.label}</button>
            </div>:<div style={{marginTop:11,fontSize:12,color:C.muted,lineHeight:1.45}}>{r.error||'O token não expõe este produto para a conta atual.'}</div>}
          </article>
        })}
      </div>
    </PageCard>
    ...
    {deploys&&<div style={{position:'fixed',inset:0,zIndex:1200,background:'#0008',display:'flex',alignItems:'center',justifyContent:'center',padding:14}} onClick={e=>e.target===e.currentTarget&&setDeploys(null)}>
      <div style={{width:'min(100%,680px)',maxHeight:'calc(100dvh - 28px)',overflow:'auto',background:C.surface,border:`1px solid ${C.border}`,borderRadius:RADIUS.lg,padding:20}}>
        ...
        {deploys.loading?<div style={{padding:35,textAlign:'center'}}><Spin size={20}/></div>:deploys.error?<p style={{color:CF.err}}>{deploys.error}</p>:(deploys.items||[]).map(d=><div key={d.id} style={{padding:'10px 0',borderTop:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',gap:12}}>
          <div><b style={{fontSize:11,color:C.text}}>{d.environment||'deployment'} · {d.latest_stage?.status||d.stages?.at?.(-1)?.status||'—'}</b><div style={{fontSize:12,color:C.muted,marginTop:3}}>{d.deployment_trigger?.metadata?.commit_message||d.id}</div></div>
          {d.url&&<a href={d.url.startsWith('http')?d.url:`https://${d.url}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:CF.orange}}>Abrir ↗</a>}
        </div>)}
      </div>
    </div>}
  </div>
...
.cf-r2-shell{display:grid;gap:12px}.cf-r2-public-card{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;border:1px solid color-mix(in srgb,var(--cf-brand,#f6821f) 30%,var(--adm-border));border-radius:14px;background:color-mix(in srgb,var(--cf-brand,#f6821f) 5%,var(--adm-surface));padding:11px 12px}.cf-r2-public-main{min-width:0;display:grid;gap:4px}.cf-r2-public-main small{font-size:11px;font-weight:900;letter-spacing:.1em;color:var(--cf-brand,#f6821f)}.cf-r2-public-main>b{font-size:11px}.cf-r2-public-main code{display:block;padding:7px 8px;border:1px solid var(--adm-border);border-radius:8px;background:var(--adm-bg);font-size:12px;overflow-wrap:anywhere;color:var(--adm-text)}.cf-r2-public-main span{font-size:12px;line-height:1.45;color:var(--adm-muted)}.cf-r2-public-actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.cf-r2-public-actions button,.cf-r2-public-actions a{border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface);color:var(--adm-text);padding:7px 9px;font:inherit;font-size:12px;font-weight:850;text-decoration:none;cursor:pointer}.cf-r2-public-actions button:disabled{opacity:.45;cursor:not-allowed}.cf-r2-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr)) auto;gap:8px;align-items:stretch}.cf-r2-stats>div,.cf-explorer-context>div{border:1px solid var(--adm-border);border-radius:12px;background:var(--adm-surface);padding:10px 12px;min-width:0}.cf-r2-stats span,.cf-explorer-context span{display:block;font-size:11px;font-weight:900;letter-spacing:.1em;color:var(--adm-muted)}.cf-r2-stats b,.cf-explorer-context b{display:block;font-size:13px;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cf-r2-stats>button{width:44px;min-height:44px;border:1px solid var(--adm-border);border-radius:12px;background:var(--adm-surface);color:var(--adm-text);font-size:18px}.cf-explorer-card,.cf-explorer-drop{border:1px solid var(--adm-border);border-radius:16px;background:var(--adm-surface);overflow:hidden}.cf-explorer-card-head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px 12px;align-items:start;padding:12px 14px;border-bottom:1px solid var(--adm-border)}.cf-explorer-card-head small{font-size:11px;font-weight:950;letter-spacing:.12em;color:var(--cf-brand)}.cf-explorer-card-head h3{font-size:16px;margin:2px 0}.cf-explorer-card-head p{font-size:12px;color:var(--adm-muted);margin:0;line-height:1.35}.cf-explorer-card-head>button{align-self:start;white-space:nowrap;padding:7px 10px;font-size:12px}.cf-explorer-card button,.cf-r2-shell button{cursor:pointer}.cf-r2-shell button.primary,.cf-explorer-form button.primary,.cf-explorer-toolbar button.primary{background:#f6821f;color:#fff;border-color:var(--cf-brand)}.cf-bucket-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;padding:12px}.cf-bucket-card{position:relative;overflow:hidden;min-height:145px;padding:12px;text-align:left;border:1px solid var(--adm-border);border-radius:14px;background:linear-gradient(145deg,var(--adm-surface),var(--adm-surface2));color:var(--adm-text)}.cf-bucket-card.default:before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;background:#f6821f}.cf-bucket-top{display:flex;justify-content:space-between;align-items:center}.cf-bucket-icon{width:34px;height:34px;border-radius:10px;display:grid;place-items:center;border:1px solid var(--adm-border);background:var(--adm-bg);color:var(--cf-brand);font-size:12px;font-weight:950}.cf-bucket-top em{font-style:normal;font-size:11px;font-weight:950;letter-spacing:.08em;color:#16a34a}.cf-bucket-card h4{margin:12px 0 6px;font-size:12px;overflow:hidden;text-overflow:ellipsis}.cf-bucket-meta{display:flex;flex-wrap:wrap;gap:5px}.cf-bucket-meta span{font-size:11px;color:var(--adm-muted);border:1px solid var(--adm-border);border-radius:999px;padding:4px 6px}.cf-bucket-actions{margin-top:12px;padding-top:9px;border-top:1px solid var(--adm-border);display:flex;align-items:center;gap:8px;font-size:11px;font-weight:850;color:var(--adm-muted)}.cf-bucket-actions span:hover{color:var(--cf-brand)}.cf-bucket-actions .danger:hover{color:#ef4444}.cf-bucket-actions b{margin-left:auto;color:var(--cf-brand);font-size:12px}.cf-explorer-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.cf-explorer-toolbar>button,.cf-explorer-toolbar-actions button,.cf-preview-actions button,.cf-explorer-more button{border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface);color:var(--adm-text);padding:8px 10px;font-size:12px;font-weight:800}.cf-explorer-toolbar-actions{margin-left:auto;display:flex;gap:6px;flex-wrap:wrap}.cf-explorer-crumb{display:flex;align-items:center;gap:4px;min-width:0;overflow:auto}.cf-explorer-crumb button{border:0;background:transparent;color:var(--cf-brand);padding:4px;font-size:12px;font-weight:800;white-space:nowrap}.cf-explorer-crumb span{color:var(--adm-muted)}.cf-explorer-context{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:7px;align-items:stretch}.cf-explorer-context>button{border:1px solid #ef444455;background:#ef44440b;color:#ef4444;border-radius:12px;padding:0 12px;font-size:12px;font-weight:900}.cf-explorer-list-head{display:flex;justify-content:space-between;align-items:center;gap:10px;padding:10px 12px;border-bottom:1px solid var(--adm-border);background:var(--adm-surface2);font-size:12px;color:var(--adm-muted)}.cf-explorer-list-head label{display:flex;align-items:center;gap:5px}.cf-object-area.list{display:block}.cf-object-area.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:10px}.cf-folder-row,.cf-object-row{width:100%;display:flex;align-items:center;gap:9px;border:0;border-bottom:1px solid var(--adm-border);border-radius:0;background:transparent;color:var(--adm-text);padding:9px 12px;text-align:left}.cf-folder-row:hover,.cf-object-row:hover{background:#f6821f0a}.cf-folder-row>span{font-size:15px}.cf-folder-row>div{display:grid;gap:2px;min-width:0}.cf-folder-row b,.cf-object-open b{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cf-folder-row small,.cf-object-open small{font-size:11px;color:var(--adm-muted)}.cf-folder-row em{margin-left:auto;font-style:normal;color:var(--cf-brand)}.cf-object-row.selected{background:#f6821f0d}.cf-object-check{display:grid;place-items:center}.cf-object-open{min-width:0;flex:1;display:flex;align-items:center;gap:8px;border:0;background:transparent;color:var(--adm-text);text-align:left;padding:0}.cf-object-open>div{display:grid;gap:2px;min-width:0}.cf-object-icon{font-size:15px}.cf-object-actions{display:flex;gap:3px;margin-left:auto}.cf-object-actions button{width:36px;height:36px;padding:0;border:1px solid var(--adm-border);border-radius:8px;background:var(--adm-surface2);color:var(--adm-muted)}.cf-object-actions .danger{color:#ef4444}.cf-object-area.grid .cf-object-row{position:relative;display:block;border:1px solid var(--adm-border);border-radius:12px;padding:8px;min-width:0}.cf-object-area.grid .cf-folder-row{border:1px solid var(--adm-border);border-radius:12px;min-height:92px}.cf-object-area.grid .cf-object-check{position:absolute;top:7px;left:7px;z-index:2;background:var(--adm-surface);border-radius:5px;padding:2px}.cf-object-area.grid .cf-object-open{display:grid;gap:7px}.cf-object-area.grid .cf-object-open img{width:100%;aspect-ratio:1.5;object-fit:cover;border-radius:8px;background:var(--adm-bg)}.cf-object-area.grid .cf-object-icon{display:grid;place-items:center;width:100%;aspect-ratio:1.5;border-radius:8px;background:var(--adm-bg);font-size:26px}.cf-object-area.grid .cf-object-actions{margin:7px 0 0;justify-content:flex-end}.cf-explorer-drop.dragging{outline:2px dashed #f6821f;outline-offset:-5px;background:#f6821f08}.cf-upload-progress{height:3px;background:var(--adm-border)}.cf-upload-progress span{display:block;height:100%;background:#f6821f;transition:width .15s}.cf-explorer-loading,.cf-explorer-empty{min-height:120px;display:grid;place-items:center;align-content:center;gap:5px;color:var(--adm-muted);font-size:12px;text-align:center;padding:18px}.cf-explorer-empty b{font-size:11px;color:var(--adm-text)}.cf-explorer-more{display:flex;justify-content:center;padding:10px}.cf-explorer-modal-bg{position:fixed;inset:0;z-index:1400;background:#0008;display:grid;place-items:center;padding:14px}.cf-explorer-modal{width:min(620px,100%);max-height:88dvh;overflow:auto;border:1px solid var(--adm-border);border-radius:18px;background:var(--adm-surface);box-shadow:0 24px 80px #0007}.cf-explorer-modal-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;align-items:center;padding:13px 15px;border-bottom:1px solid var(--adm-border);background:var(--adm-surface)}.cf-explorer-modal-head b{font-size:13px}.cf-explorer-modal-head button{border:0;background:transparent;color:var(--adm-muted);font-size:20px}.cf-explorer-form{padding:15px;display:grid;gap:8px}.cf-explorer-form label{font-size:12px;font-weight:900;color:var(--adm-muted)}.cf-explorer-form input{width:100%;box-sizing:border-box;padding:10px;border:1px solid var(--adm-border);border-radius:10px;background:var(--adm-bg);color:var(--adm-text)}.cf-explorer-form small{font-size:12px;color:var(--adm-muted)}.cf-explorer-form>div{display:flex;justify-content:flex-end;gap:7px;margin-top:6px}.cf-explorer-form button{border:1px solid var(--adm-border);border-radius:9px;padding:8px 11px;background:var(--adm-surface2);color:var(--adm-text);font-size:12px;font-weight:800}.cf-preview-body{padding:14px;display:grid;gap:12px}.cf-preview-body>img,.cf-preview-body>video{width:100%;max-height:55vh;object-fit:contain;border-radius:12px;background:var(--adm-bg)}.cf-preview-body>audio{width:100%}.cf-file-generic{min-height:160px;border:1px dashed var(--adm-border);border-radius:12px;background:var(--adm-bg);display:grid;place-items:center;align-content:center;gap:6px;font-size:34px}.cf-file-generic b{font-size:11px}.cf-file-generic span{font-size:12px;color:var(--adm-muted)}.cf-preview-info{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}.cf-preview-info span{display:grid;gap:2px;padding:8px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2);font-size:12px;overflow-wrap:anywhere}.cf-preview-info b{font-size:11px;color:var(--adm-muted)}.cf-preview-actions{display:flex;justify-content:flex-end;gap:6px}
@media(max-width:760px){.cf-r2-public-card{grid-template-columns:1fr}.cf-r2-public-actions{justify-content:flex-start}.cf-bucket-grid{grid-template-columns:repeat(2,minmax(0,1fr));padding:8px}.cf-object-area.grid{grid-template-columns:repeat(2,minmax(0,1fr));padding:7px}.cf-explorer-toolbar-actions{width:100%;margin-left:0;display:grid;grid-template-columns:repeat(4,minmax(0,1fr))}.cf-explorer-toolbar-actions button{padding:8px 5px}.cf-explorer-context{grid-template-columns:repeat(3,minmax(0,1fr))}.cf-explorer-context>div:first-child{grid-column:1/-1}.cf-explorer-context>button{grid-column:1/-1;min-height:38px}.cf-explorer-list-head{align-items:flex-start}.cf-r2-stats{grid-template-columns:repeat(3,minmax(0,1fr))}.cf-r2-stats>button{grid-column:1/-1;width:100%;height:44px}.cf-explorer-card-head{grid-template-columns:minmax(0,1fr) auto;align-items:start;gap:7px}.cf-explorer-card-head>button{width:auto;min-width:0;padding:7px 8px}.cf-explorer-card-head p{grid-column:1/-1}.cf-preview-info{grid-template-columns:1fr}.cf-explorer-modal-bg{padding:10px}.cf-explorer-modal{max-height:calc(100dvh - 20px)}}
`
                ...
                {eventos.map((e, i) => {
                  ...
                  return (
                    <tr key={e.id || i} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: '7px 10px', fontFamily: 'monospace', color: C.muted, fontSize: 12 }}>
                        {e.rule_id || e.ruleId || '—'}
                      </td>
                    </tr>
                  )
...
export default function AbaCloudflare() {
  ...
  const rawAccountName=String(dashboard?.account?.name||statusCF?.conta?.name||'').trim()
  const accountDisplay=(()=>{
    if(!rawAccountName)return 'Sua conta Cloudflare'
    const emailMatch=rawAccountName.match(/^([^@]+)@[^']+(?:'s Account)?$/i)
    if(emailMatch)return `Conta de ${emailMatch[1]}`
    return rawAccountName.replace(/'s Account$/i,'').trim()||rawAccountName
  })()
  const accountDetail=rawAccountName&&rawAccountName!==accountDisplay?rawAccountName.replace(/'s Account$/i,'').trim():''
  const r2Ready=Boolean(statusCF?.s3Credentials?.configurado&&statusCF?.s3Credentials?.valido)
  const tools=[
  ...
  const ToolContent=()=>{
    ...
    if(tool==='account')return <div className="cf-account-panel">
      <section><small>CONTA ATIVA</small><h3>{accountDisplay}</h3>{accountDetail&&<p>{accountDetail}</p>}<p>{dashboard?.account?.id||statusCF?.account_id||'—'}</p></section>
  ...
  return <div className="cf-central">
    <section className="cf-central-hero">
      <div className="cf-central-brand"><span className="cf-central-logo">CF</span><div><small>CLOUDFLARE CENTRAL</small><h2>{loadingStatus?'Carregando…':accountDisplay}</h2><p>{tokenAtivo?(accountDetail?`${accountDetail} · API conectada`:'API conectada e pronta para administrar recursos'):'Conecte a Cloudflare em Integrações e APIs'}</p></div></div>
    ...
    <section className="cf-central-stats">
      <button onClick={()=>{setSettingsOpen(true);setTool('account')}}><small>CONTA</small><b>{accountDisplay}</b><span>{dashboard?.accounts?.length>1?`${dashboard.accounts.length} contas acessíveis`:(accountDetail||'Conta ativa')}</span></button>
      ...
      <div><small>R2</small><b>{r2Ready?'Pronto':statusCF?.s3Credentials?.configurado?'Revisar':'Configurar'}</b><span>{r2Ready?'S3 conectado':statusCF?.s3Credentials?.configurado?'Credenciais precisam de revisão':'Chaves S3 não configuradas'}</span></div>
    </section>
