...
function SetupPerformanceSummary(){
 ...
 return <div style={{margin:'0 0 18px',padding:'12px 14px',borderRadius:RADIUS.lg,border:`1px solid ${slow?SETUP.warning+'55':SETUP.success+'33'}`,background:slow?`${SETUP.warning}0d`:SETUP.successSoft}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}>
   <div><div style={{fontSize:12,fontWeight:900,color:SETUP.text}}>Diagnóstico da inicialização</div><div style={{fontSize:12,color:SETUP.muted,marginTop:3}}>HTTP <strong>{fmt(httpMs)}</strong> • backend <strong>{fmt(serverMs)}</strong>{chunkMs?<> • módulo <strong>{fmt(chunkMs)}</strong></>:null}</div></div>
  ...
  {httpMs>700&&serverMs<100&&<div style={{fontSize:12,color:SETUP.warning,marginTop:8,lineHeight:1.45}}>A rota respondeu rápido no servidor, mas a ida/volta HTTP demorou. O gargalo tende a estar no Vite, navegador ou comunicação local.</div>}
  {serverMs>700&&<div style={{fontSize:12,color:SETUP.warning,marginTop:8,lineHeight:1.45}}>O próprio backend demorou para calcular o estado do setup. Vamos investigar leitura do cofre/armazenamento local.</div>}
  {chunkMs>700&&<div style={{fontSize:12,color:SETUP.warning,marginTop:8,lineHeight:1.45}}>O módulo do assistente demorou para ser processado pelo Vite. Este é um forte candidato à lentidão no Termux.</div>}
 </div>
}
