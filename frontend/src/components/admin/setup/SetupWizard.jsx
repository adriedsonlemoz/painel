...
export default function SetupWizard({statusBanco,onSucesso}){
 ...
 const [mongo,setMongo]=useState(statusBanco?.mongo_conectado?{ok:true,banco:statusBanco?.banco_nome}:null),[mongoLoading,setMongoLoading]=useState(false),[atlasImport,setAtlasImport]=useState(null),[acaoExistente,setAcaoExistente]=useState('')
 ...
 async function adotarExistente(){setAcaoExistente('adotar');try{const res=await setupService.adotarInstalacao(mongoPayload());toast.success(res.mensagem||'Instalação existente vinculada.');window.location.href='/admin'}catch(err){toast.error(err.message||'Não foi possível reutilizar a instalação.')}finally{setAcaoExistente('')}}
 ...
 async function instalar(){setInstalando(true);try{const res=await setupService.instalar({nome:form.nome,email:form.email,senha:form.senha,nome_site:form.nomeSite,organizacao:form.organizacao,importar_seed:seed,dados_escolhidos:seed?dadosSel:[]});onSucesso(res)}catch(err){toast.error(err.message||'Erro na instalação')}finally{setInstalando(false)}}
 ...
 {etapa===5&&<><div style={{display:'grid',gap:10}}>{[['Tipo de banco',providerLabel],['Banco de dados',dbName||'alsistemas'],['Organização',form.organizacao],['Nome do site',form.nomeSite],['Administrador',`${form.nome} • ${form.email}`],['Dados iniciais',resumoSeed]].map(([a,b])=><div key={a} style={{display:'flex',justifyContent:'space-between',gap:18,padding:'13px 14px',background:SETUP.input,border:`1px solid ${SETUP.border}`,borderRadius:RADIUS.md,fontSize:13}}><span style={{color:SETUP.muted}}>{a}</span><strong style={{textAlign:'right',color:SETUP.text}}>{b}</strong></div>)}</div><div style={{marginTop:16}}><Notice>A origem do MongoDB pode ser alterada depois no painel sem mudar os modelos do sistema.</Notice></div><Nav etapa={etapa} onVoltar={()=>setEtapa(4)} onAvancar={instalar} avancarLabel="Instalar AL Sistemas" loading={instalando}/></>}
 </section><p style={{textAlign:'center',color:SETUP.muted,fontSize:11,marginTop:16}}>MongoDB Atlas ou VPS • segredos armazenados no cofre local do servidor</p></div></div>
}
