...
export default function NovoProjetoGitHubWizard({status,onClose,onCreated}){
  ...
  const [storageHealth,setStorageHealth]=useState({loading:false,ok:null,message:''})
  ...
  useEffect(()=>{
    if(step!==3)return
    let alive=true
    setStorageHealth({loading:true,ok:null,message:'Verificando disponibilidade…'})
    const check=storage==='r2'?projetosService.r2Health():projetosService.gridfsHealth()
    check.then(d=>{if(alive)setStorageHealth({loading:false,ok:Boolean(d?.ok),message:d?.ok?(d.mensagem||`${storage==='r2'?'R2':'GridFS'} disponível.`):(d?.erro||'Armazenamento indisponível.')})})
      .catch(e=>{if(alive)setStorageHealth({loading:false,ok:false,message:e.message||'Não foi possível verificar o armazenamento.'})})
    return()=>{alive=false}
  },[step,storage])
  useEffect(()=>{
  ...
  async function upload(){
    ...
    if(storageHealth.loading)return setError('Aguarde a verificação do armazenamento escolhido.')
    if(storageHealth.ok===false)return setError(`${storage==='r2'?'Cloudflare R2':'MongoDB GridFS'} indisponível: ${storageHealth.message}`)
    ...
    }catch(e){setError(`O repositório ${repo?.nomeCompleto || ''} já foi criado, mas a preparação do pacote falhou. ${e.message || 'Tente novamente.'}`.trim())}finally{setUploading(false)}
  }
    ...
    <div className="gh-create-modal" role="dialog" aria-modal="true" aria-label="Novo projeto GitHub">
      ...
      <div className="gh-create-body">
        ...
        {step===2&&<div className="gh-create-step">
          <p>Complete os dados do GitHub. Ao continuar, o repositório será criado e inicializado com a branch padrão para que o primeiro envio não falhe.</p>
          <div className="gh-create-form one-screen">
        ...
        {step===3&&<div className="gh-create-step">
          <div className="gh-create-created"><span>✓</span><div><b>{repo?.nomeCompleto}</b><small>Repositório criado e pronto para receber commits. Agora escolha onde o ZIP será preparado antes de ser enviado ao GitHub.</small></div></div>
          <div className="gh-create-storage-intro"><b>Onde preparar o pacote?</b><span>Essa escolha não muda o destino final: o código continuará indo para o GitHub. R2/GridFS apenas guardam os arquivos usados durante a preparação e permitem retomar o projeto pelo painel.</span></div>
          <div className="gh-create-storage">
            <button className={storage==='r2'?'active':''} onClick={()=>{setError('');setStorage('r2')}}><span className="gh-create-storage-title"><b>☁️ Cloudflare R2</b><em>{storage==='r2'?(storageHealth.loading?'verificando':storageHealth.ok===true?'pronto':storageHealth.ok===false?'indisponível':'selecionado'):''}</em></span><small>Melhor para ambiente cloud e pacotes maiores. Exige bucket e credenciais S3 do R2 configuradas.</small></button>
            <button className={storage==='gridfs'?'active':''} onClick={()=>{setError('');setStorage('gridfs')}}><span className="gh-create-storage-title"><b>▦ MongoDB GridFS</b><em>{storage==='gridfs'?(storageHealth.loading?'verificando':storageHealth.ok===true?'pronto':storageHealth.ok===false?'indisponível':'selecionado'):''}</em></span><small>Guarda o pacote no MongoDB. Útil quando você já usa o banco e não quer depender do R2.</small></button>
          </div>
          <div className={`gh-create-storage-status ${storageHealth.ok===false?'error':storageHealth.ok===true?'ok':''}`}><b>{storage==='r2'?'R2':'GridFS'} · {storageHealth.loading?'verificando':storageHealth.ok===true?'disponível':storageHealth.ok===false?'ação necessária':'aguardando'}</b><span>{storageHealth.message || 'A verificação acontece automaticamente antes do upload.'}</span></div>
          ...
          {uploading&&<div className="gh-create-upload"><div><span>{uploadProgress.phase==='upload'?'1/3 · Enviando ZIP ao painel':uploadProgress.phase==='done'?'3/3 · Pacote preparado':'2/3 · Descompactando e salvando arquivos'}</span><b>{uploadProgress.percent||0}%</b></div><i><em style={{width:`${uploadProgress.percent||0}%`}}/></i><small>Depois desta etapa o painel vincula o projeto ao repositório e abre a revisão do primeiro commit.</small></div>}
          <div className="gh-create-actions"><DSBtn variant="primary" onClick={upload} loading={uploading} disabled={!file||storageHealth.loading||storageHealth.ok===false}>Enviar e preparar →</DSBtn></div>
        </div>}
      ...
      <style>{`
        .gh-create-overlay{position:fixed;inset:0;z-index:1800;background:rgba(15,23,42,.48);backdrop-filter:blur(3px);display:grid;place-items:center;padding:14px}.gh-create-modal{width:min(620px,100%);max-height:min(86dvh,700px);display:flex;flex-direction:column;overflow:hidden;background:var(--adm-surface);border:1px solid var(--adm-border);border-radius:20px;box-shadow:0 28px 80px rgba(15,23,42,.28)}.gh-create-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:14px 17px 9px;border-bottom:1px solid var(--adm-border)}.gh-create-head small{font-size:11px;font-weight:900;letter-spacing:.14em;color:var(--adm-muted)}.gh-create-head h2{margin:3px 0 0;font-size:17px;color:var(--adm-text)}.gh-create-head button{border:0;background:transparent;color:var(--adm-muted);font-size:22px;cursor:pointer}.gh-create-dots{display:grid;grid-template-columns:repeat(6,1fr);gap:5px;padding:8px 17px}.gh-create-dots span{height:23px;border-radius:7px;display:grid;place-items:center;background:var(--adm-surface2);border:1px solid var(--adm-border);font-size:11px;font-weight:900;color:var(--adm-muted)}.gh-create-dots span.active{border-color:var(--adm-accent);color:var(--adm-accent)}.gh-create-dots span.done{background:color-mix(in srgb,var(--adm-green,#22c55e) 10%,var(--adm-surface));color:var(--adm-green,#16a34a)}.gh-create-body{overflow:auto;padding:0 17px 16px}.gh-create-step{display:grid;gap:11px;align-content:start}.gh-create-step>p{margin:0;color:var(--adm-muted);font-size:11px;line-height:1.45}.gh-create-form,.gh-create-review-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.gh-create-form label,.gh-create-review-grid label{display:grid;gap:4px}.gh-create-form label.wide,.gh-create-review-grid label.wide{grid-column:1/-1}.gh-create-form span,.gh-create-review-grid span{font-size:11px;font-weight:850;color:var(--adm-muted);text-transform:uppercase;letter-spacing:.05em}.gh-create-form input,.gh-create-form select,.gh-create-form textarea,.gh-create-review-grid input{width:100%;box-sizing:border-box;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2);color:var(--adm-text);padding:9px;font:inherit;font-size:11px;outline:none}.gh-create-visibility{display:flex;align-items:center;gap:9px;text-align:left;padding:10px;border:1px solid var(--adm-border);border-radius:10px;background:var(--adm-surface2);color:var(--adm-text);cursor:pointer}.gh-create-visibility>span{font-size:18px}.gh-create-visibility>div{display:grid;gap:1px}.gh-create-visibility b{font-size:12px}.gh-create-visibility small{font-size:11px;color:var(--adm-muted)}.gh-create-options{display:grid;grid-template-columns:1fr 1fr;gap:6px}.gh-create-options label{display:flex;align-items:center;gap:7px;padding:8px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2)}.gh-create-options b{font-size:12px;color:var(--adm-text)}.gh-create-actions{display:flex;justify-content:flex-end;gap:7px;align-items:center;position:sticky;bottom:-1px;background:var(--adm-surface);padding-top:8px;padding-bottom:2px;z-index:3}.gh-create-created{display:grid;grid-template-columns:30px minmax(0,1fr);gap:9px;align-items:center;padding:9px;border:1px solid color-mix(in srgb,var(--adm-green,#22c55e) 35%,var(--adm-border));border-radius:10px;background:color-mix(in srgb,var(--adm-green,#22c55e) 7%,var(--adm-surface))}.gh-create-created>span{width:29px;height:29px;display:grid;place-items:center;border-radius:9px;background:var(--adm-green,#22c55e);color:white;font-weight:900}.gh-create-created div{display:grid;gap:1px;min-width:0}.gh-create-created b{font-size:12px;color:var(--adm-text);overflow-wrap:anywhere}.gh-create-created small{font-size:11px;color:var(--adm-muted);line-height:1.35}.gh-create-storage-intro{display:grid;gap:3px;padding:9px;border-radius:10px;border:1px solid var(--adm-border);background:var(--adm-surface2)}.gh-create-storage-intro b{font-size:12px;color:var(--adm-text)}.gh-create-storage-intro span{font-size:11px;line-height:1.4;color:var(--adm-muted)}.gh-create-storage{display:grid;grid-template-columns:1fr 1fr;gap:7px}.gh-create-storage button{display:grid;gap:5px;text-align:left;padding:10px;border-radius:10px;border:1px solid var(--adm-border);background:var(--adm-surface2);color:var(--adm-text);cursor:pointer}.gh-create-storage button.active{border-color:var(--adm-accent);box-shadow:inset 0 0 0 1px var(--adm-accent)}.gh-create-storage-title{display:flex;align-items:center;justify-content:space-between;gap:6px}.gh-create-storage b{font-size:12px}.gh-create-storage em{font-style:normal;font-size:11px;font-weight:900;text-transform:uppercase;color:var(--adm-accent)}.gh-create-storage small{font-size:11px;line-height:1.35;color:var(--adm-muted)}.gh-create-storage-status{display:grid;gap:2px;padding:8px 9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2)}.gh-create-storage-status b{font-size:11px;color:var(--adm-text)}.gh-create-storage-status span{font-size:11px;line-height:1.35;color:var(--adm-muted)}.gh-create-storage-status.ok{border-color:color-mix(in srgb,var(--adm-green,#22c55e) 35%,var(--adm-border))}.gh-create-storage-status.error{border-color:color-mix(in srgb,var(--adm-red,#ef4444) 42%,var(--adm-border))}.gh-create-storage-status.error b{color:var(--adm-red,#dc2626)}.gh-create-drop{width:100%;display:grid;place-items:center;gap:3px;padding:15px 10px;border:1.5px dashed var(--adm-border);border-radius:11px;background:var(--adm-surface2);color:var(--adm-text);cursor:pointer}.gh-create-drop.selected{border-color:var(--adm-accent)}.gh-create-drop>span{font-size:20px}.gh-create-drop>b{font-size:12px}.gh-create-drop>small{font-size:11px;color:var(--adm-muted)}.gh-create-upload>div{display:flex;justify-content:space-between;font-size:11px;color:var(--adm-muted);margin-bottom:4px}.gh-create-upload>small{display:block;margin-top:5px;font-size:11px;line-height:1.35;color:var(--adm-muted)}.gh-create-upload>i,.gh-create-big-progress{display:block;height:7px;border-radius:999px;background:var(--adm-surface2);overflow:hidden}.gh-create-upload em,.gh-create-big-progress i{display:block;height:100%;background:var(--adm-accent);border-radius:inherit;transition:width .3s}.gh-create-file-preview{max-height:104px;overflow:auto;border:1px solid var(--adm-border);border-radius:9px;padding:8px;display:grid;gap:3px;background:var(--adm-surface2)}.gh-create-file-preview>div{display:flex;justify-content:space-between;color:var(--adm-text);font-size:12px}.gh-create-file-preview code{font-size:11px;color:var(--adm-muted);overflow-wrap:anywhere}.gh-create-file-preview small{font-size:11px;color:var(--adm-accent);font-weight:800}.gh-create-destination{display:grid;gap:2px;padding:9px;border-radius:9px;background:var(--adm-surface2);border:1px solid var(--adm-border)}.gh-create-destination span{font-size:11px;color:var(--adm-muted);text-transform:uppercase;font-weight:850}.gh-create-destination b{font-size:12px;color:var(--adm-text);overflow-wrap:anywhere}.gh-create-commit,.gh-create-finish{text-align:center;justify-items:center;padding:8px 0}.gh-create-stage-icon,.gh-create-finish-icon{width:48px;height:48px;display:grid;place-items:center;border-radius:15px;background:color-mix(in srgb,var(--adm-accent) 10%,var(--adm-surface2));color:var(--adm-accent);font-size:23px;font-weight:900}.gh-create-finish-icon{background:color-mix(in srgb,var(--adm-green,#22c55e) 12%,var(--adm-surface2));color:var(--adm-green,#16a34a)}.gh-create-commit h3,.gh-create-finish h3{margin:0;font-size:15px;color:var(--adm-text)}.gh-create-commit>p,.gh-create-finish>p{margin:0;max-width:420px}.gh-create-big-progress{width:min(410px,100%);height:9px}.gh-create-commit>strong{font-size:18px;color:var(--adm-accent)}.gh-create-inline-error{padding:8px 9px;border:1px solid color-mix(in srgb,var(--adm-red,#ef4444) 45%,var(--adm-border));border-radius:9px;background:color-mix(in srgb,var(--adm-red,#ef4444) 8%,var(--adm-surface));color:var(--adm-red,#dc2626);font-size:12px;line-height:1.4}.gh-create-link{display:inline-flex;align-items:center;min-height:33px;padding:0 11px;border-radius:9px;border:1px solid var(--adm-border);color:var(--adm-accent);font-size:12px;font-weight:800;text-decoration:none}
        @media(max-width:560px){.gh-create-overlay{padding:4px;place-items:center}.gh-create-modal{width:100%;max-height:calc(100dvh - 8px);border-radius:15px}.gh-create-head{padding:11px 12px 7px}.gh-create-head h2{font-size:16px}.gh-create-dots{padding:7px 12px;gap:3px}.gh-create-dots span{height:20px;font-size:11px}.gh-create-body{padding:0 12px 11px}.gh-create-form{grid-template-columns:1fr}.gh-create-review-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.gh-create-form label.wide{grid-column:auto}.gh-create-review-grid label.wide{grid-column:1/-1}.gh-create-options{grid-template-columns:1fr 1fr;gap:4px}.gh-create-options label{padding:6px}.gh-create-options b{font-size:11px}.gh-create-storage button{padding:8px}.gh-create-storage{grid-template-columns:1fr}.gh-create-storage small{display:block}.gh-create-storage-intro span{font-size:11px}.gh-create-drop{padding:12px 8px}.gh-create-actions>*{flex:1;justify-content:center}.gh-create-file-preview{max-height:86px}.gh-create-step{gap:9px}}
        @media(max-height:620px){.gh-create-modal{max-height:94dvh}.gh-create-head{padding-top:8px}.gh-create-dots{padding-top:5px;padding-bottom:5px}.gh-create-body{padding-bottom:8px}.gh-create-step{gap:7px}.gh-create-step>p{font-size:12px}.gh-create-drop{padding:9px}.gh-create-file-preview{max-height:70px}}
      `}</style>
    </div>
  </div>
}
