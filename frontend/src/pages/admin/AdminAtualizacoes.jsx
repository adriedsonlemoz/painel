...
export default function AdminAtualizacoes(){
  ...
  return <div className="adm-page updates-hub" style={{display:'grid',gap:14}}>
    ...
    <style>{`
      ...
      @media(max-width:390px){.updates-command small{font-size:9px}.updates-command b{font-size:12px}.updates-overview>div{padding:9px}.updates-recovery-actions{grid-template-columns:1fr}}
    `}</style>
  </div>
...
function UpdateProgressModal({job,onClose}){
  ...
  return <div className="updates-modal-overlay updates-progress-overlay" role="presentation" onMouseDown={e=>{if(canClose&&e.target===e.currentTarget)onClose?.()}}>
    <div className="updates-modal updates-progress-modal" role="dialog" aria-modal="true" aria-labelledby="update-progress-title">
      ...
      <div className="updates-progress-footer updates-progress-footer-clean"><span>{canClose?(job.status==='completed'?'Atualização concluída. Use × no topo para fechar.':job.status==='failed'?'A atualização terminou com erro. Use × no topo para fechar.':'Operação finalizada. Use × no topo para fechar.'):'Não feche esta aba durante a aplicação dos arquivos.'}</span></div>
    </div>
  </div>
}
