...
export default function AdminIntegracoes(){
 const [status,setStatus]=useState(null),[tab,setTab]=useState(null),[form,setForm]=useState(blank),[mongo,setMongo]=useState({uri:'',databaseName:''}),[busy,setBusy]=useState(false),[diag,setDiag]=useState(null),[aiDiag,setAiDiag]=useState(null),[exportSecrets,setExportSecrets]=useState(false),[exporting,setExporting]=useState(false),[utility,setUtility]=useState(null),[importFile,setImportFile]=useState(null),[importing,setImporting]=useState(false)
 ...
 return <div className="adm-page integrations-page" style={{color:'var(--adm-text)'}}>
  ...
  {utility==='ai-diagnostics'&&<Modal title="Diagnóstico da IA" subtitle="Gemini e OpenRouter · geração de texto + JSON estruturado" onClose={()=>setUtility(null)}>
   ...
   <div className="ai-diagnostic-list">{(aiDiag?.providers||[]).map(p=><div key={p.id} className="ai-diagnostic-provider"><div><b>{p.nome}</b><span>{p.model||p.modelo||'—'}</span></div><strong className={p.ok?'ok':'warn'}>{p.ok?'Funcionando':p.configured?'Com problema':'Não configurado'}</strong><p>{p.status}</p>{p.configured&&<div className="ai-diagnostic-tests"><span>{p.textOk===true?'✓':p.textOk===false?'✕':'○'} Texto</span><span>{p.structuredOk===true?'✓':p.structuredOk===false?'✕':'○'} JSON estruturado</span>{p.latencyMs!=null&&<span>{p.latencyMs} ms</span>}</div>}</div>)}</div>
   <p className="modal-copy" style={{marginTop:12}}>Se o provedor principal falhar durante um recurso do AL, o núcleo tenta o outro provedor ativo automaticamente. Falhas também são registradas na Central de Erros.</p>
   <div className="modal-actions"><button className="primary" onClick={runAiDiag} disabled={busy}>{busy?'Testando…':'Testar novamente'}</button><button onClick={()=>setUtility(null)}>Fechar</button></div>
  </Modal>}
