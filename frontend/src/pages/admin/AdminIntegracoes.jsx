...
import { DSPageHeader, DSBtn, DSModal } from '../../components/admin/ui/DS'
...
const integrationHelp={
 ...
 gemini:{
  ...
  expected:'API key do Google AI Studio • modelo recomendado: gemini-3.5-flash-lite',
 },
 openrouter:{
  title:'OpenRouter',
...
export default function AdminIntegracoes(){
 ...
 useEffect(()=>{const requested=new URLSearchParams(window.location.search).get('open');if(requested&&(['mongodb',...providers.map(p=>p[0])].includes(requested)))setTab(requested)},[])
 ...
 const runDiag=async()=>{setBusy(true);try{setDiag(await API('/diagnostics/run'));setUtility('general-diagnostics')}catch(e){toast.error(e.message)}finally{setBusy(false)}}
 ...
 useEffect(()=>{ if(tab==='mongodb'||tab==='github')return; const m={...(status?.integrations?.[tab]?.metadata||{})}; if(AI_PROVIDERS.includes(tab))m.model=normalizeAiModel(tab,m.model); setForm(f=>({...blank,secret:'',metadata:{...blank.metadata,apiUrl:API_DEFAULTS[tab]||'',...m}})) },[tab,status])
 return <div className="adm-page integrations-page" style={{color:'var(--adm-text)'}}>
  <DSPageHeader
   title="Integrações e APIs"
   sub="Conexões, credenciais e diagnósticos centralizados em uma única área."
   actions={<><DSBtn onClick={runAiDiag} disabled={busy}>Diagnóstico da IA</DSBtn><DSBtn variant="ghost" onClick={runDiag} disabled={busy}>Diagnóstico geral</DSBtn></>}
  />
  ...
  {status?.systemSecrets?.length>0&&<section className="system-secret-panel"><div><b>Segredos do ambiente do backend</b><span>Variáveis sensíveis detectadas fora das integrações. Permanecem mascaradas e são recuperadas individualmente.</span></div><div className="system-secret-grid">{status.systemSecrets.map(x=><SystemSecretRow key={x.name} item={x}/>)}</div></section>}

  <section className="integration-hub">
   <div className="hub-heading"><div><div className="eyebrow">SUAS CONEXÕES</div><h2>Escolha uma integração</h2><p>Grade adaptável: três cards no desktop, dois no tablet e um no celular, sem apertar o conteúdo.</p></div><DSBtn size="sm" onClick={()=>refreshIdentities(false)} disabled={identityRefreshing}>{identityRefreshing?'Atualizando…':'Atualizar status'}</DSBtn></div>
  ...
  {tab&&<Modal title={tab==='mongodb'?'MongoDB':providers.find(x=>x[0]===tab)?.[1]} subtitle={current?.locked?'Credencial incompatível':current?.configured?'Configurado':'Não configurado'} onClose={closeIntegration}>
    ...
    {current?.configured&&!current?.locked&&<StoredCredentialInspector id={tab} current={current}/>}
    {tab==='mongodb'?<><div className="integration-status-grid">
     ...
     :<><SecretField label="API Secret" value={form.secret} onChange={v=>setForm({...form,secret:v})} placeholder={current?.configured?'Digite somente para substituir':'Cole a credencial'}/>{tab==='cloudinary'&&<><Field label="Cloud Name" value={form.metadata.cloudName} onChange={v=>setForm({...form,metadata:{...form.metadata,cloudName:v}})} placeholder="ex.: meu-cloud"/><Field label={current?.fields?.apiKey?.configured?'Nova API Key (deixe vazio para manter)':'API Key'} value={form.metadata.apiKey} onChange={v=>setForm({...form,metadata:{...form.metadata,apiKey:v}})} placeholder={current?.fields?.apiKey?.configured?'Digite somente para substituir':'API Key do Console'}/></>}</>}
  ...
  {utility==='ai-diagnostics'&&<Modal title="Central de diagnóstico da IA" subtitle="Provedores, fila, fallback e consumo em um único lugar" onClose={()=>setUtility(null)}>
   <div className={`ai-diagnostic-hero ${aiDiag?.ok?'ok':'warn'}`}>
    <div className="ai-diagnostic-hero-icon">{aiDiag?.ok?'✓':'!'}</div>
    <div><small>ESTADO DO MOTOR</small><h3>{aiDiag?.ok?'IA pronta para uso':'Há pontos que precisam de atenção'}</h3><p>{aiDiag?.status||'Diagnóstico indisponível'}</p></div>
    <span>{(aiDiag?.providers||[]).filter(p=>p.ok).length}/{(aiDiag?.providers||[]).length||0} provedores</span>
   </div>
   <div className="ai-runtime-metrics ai-runtime-metrics-top"><StatusBox label="IA principal" value={aiDiag?.principal?`${aiDiag.principal.nome} · ${aiDiag.principal.modelo}`:'Não definida'}/><StatusBox label="Fila" value={`${aiDiag?.queue?.active||0} ativa · ${aiDiag?.queue?.pending||0} aguardando`}/><StatusBox label="Concorrência" value={`${aiDiag?.queue?.concurrency||2}`}/><StatusBox label="Timeout" value={`${Math.round((aiDiag?.timeoutMs||0)/1000)} s`}/></div>
   <div className="ai-diagnostic-list">{(aiDiag?.providers||[]).map(p=><article key={p.id} className={`ai-diagnostic-provider ${p.ok?'provider-ok':p.configured?'provider-warn':'provider-off'}`}>
    <div className="ai-provider-head"><div className="ai-provider-mark">{p.id==='gemini'?'G':'OR'}</div><div><b>{p.nome}</b><span>{p.model||p.modelo||'Modelo não definido'}</span></div><strong>{p.ok?'OPERACIONAL':p.configured?'ATENÇÃO':'NÃO CONFIGURADO'}</strong></div>
    <p>{p.status}</p>
    <div className="ai-diagnostic-tests"><span>{p.structuredOk===true?'✓ Resposta estruturada':p.structuredOk===false?'✕ JSON estruturado':'○ Teste rápido'}</span>{p.latencyMs!=null&&<span>◷ {p.latencyMs} ms</span>}{p.circuitOpen&&<span>⏸ cooldown {formatCooldown(p.retryAfterMs)}</span>}{p.configured&&!p.circuitOpen&&<span>✓ runtime liberado</span>}</div>
    <button type="button" className="ai-provider-open" onClick={()=>{setUtility(null);openIntegration(p.id)}}>{p.configured?'Configurar provedor':'Adicionar credencial'} →</button>
   </article>)}</div>
   {aiUsage&&<section className="ai-usage-panel"><div className="ai-usage-head"><div><small>ÚLTIMOS 7 DIAS</small><b>Uso do motor editorial</b></div><span>{aiUsage.total||0} solicitações</span></div><div className="ai-runtime-metrics"><StatusBox label="Sucesso" value={aiUsage.success||0}/><StatusBox label="Fallbacks" value={aiUsage.fallbacks||0}/><StatusBox label="Erros" value={aiUsage.errors||0}/><StatusBox label="Cache" value={aiUsage.cacheHits||0}/><StatusBox label="Latência média" value={`${Math.round(aiUsage.avgLatencyMs||0)} ms`}/><StatusBox label="Tokens" value={`${(aiUsage.inputTokens||0)+(aiUsage.outputTokens||0)}`}/><StatusBox label="Custo informado" value={`$${Number(aiUsage.costUsd||0).toFixed(4)}`}/></div></section>}
   <p className="modal-copy ai-diagnostic-note">O teste profundo faz uma geração estruturada por provedor. O diagnóstico normal do sistema continua leve e não gasta uma geração a cada atualização do painel.</p>
   <div className="modal-actions"><button className="primary" onClick={runAiDiag} disabled={busy}>{busy?'Executando testes…':'Executar diagnóstico novamente'}</button><button onClick={async()=>{await API('/ai/runtime/reset',{method:'POST',body:'{}'});toast.success('Cooldowns reiniciados');runAiDiag()}}>Liberar cooldowns</button><button onClick={()=>setUtility(null)}>Fechar</button></div>
  </Modal>}

  {utility==='general-diagnostics'&&diag&&<Modal title="Diagnóstico geral" subtitle="Estado das integrações e dependências do painel" onClose={()=>setUtility(null)}><div className="integration-diagnostic-list">{diag.checks.map(c=><div key={c.name} className={c.ok?'ok':'warn'}><span>{c.ok?'✓':'!'}</span><div><b>{c.name}</b><small>{c.detail||'OK'}</small></div></div>)}</div><div className="modal-actions"><DSBtn onClick={runDiag} disabled={busy}>{busy?'Verificando…':'Verificar novamente'}</DSBtn><DSBtn variant="ghost" onClick={()=>setUtility(null)}>Fechar</DSBtn></div></Modal>}
  <style>{`
    .integrations-titlebar,.hub-heading{display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap}.integrations-titlebar h1,.hub-heading h2{margin:4px 0 6px}.integrations-titlebar p,.hub-heading p,.modal-copy{margin:0;color:var(--adm-muted);font-size:13px;line-height:1.5}.eyebrow{font-size:12px;font-weight:900;letter-spacing:.11em;color:var(--adm-accent)}
    ...
    .integration-hub{padding:18px;border:1px solid var(--adm-border);border-radius:18px;background:var(--adm-surface)}.integration-card-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:18px}.big-integration-card{position:relative;overflow:hidden;min-width:0;min-height:178px;padding:16px;border:1px solid var(--adm-border);border-radius:16px;background:linear-gradient(145deg,var(--adm-surface),var(--adm-surface2));text-align:left;display:flex;flex-direction:column;align-items:stretch;box-shadow:0 10px 30px rgba(15,23,42,.04);transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}.big-integration-card:hover{transform:translateY(-2px);border-color:color-mix(in srgb,var(--adm-accent) 48%,var(--adm-border));box-shadow:0 16px 34px rgba(15,23,42,.09)}.integration-card-accent{position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--adm-border)}.big-integration-card.connected .integration-card-accent{background:var(--adm-accent)}.big-integration-card.paused .integration-card-accent{background:var(--adm-amber)}.big-integration-card .card-top{display:flex;align-items:center;justify-content:space-between;gap:10px}.integration-card-icon{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;border:1px solid var(--adm-border);background:var(--adm-bg);font-size:12px;font-weight:950;letter-spacing:-.02em;color:var(--adm-muted)}.big-integration-card.connected .integration-card-icon{color:var(--adm-accent);border-color:color-mix(in srgb,var(--adm-accent) 30%,var(--adm-border));background:color-mix(in srgb,var(--adm-accent) 6%,var(--adm-bg))}.big-integration-card .state{font-size:12px;font-weight:950;letter-spacing:.08em;color:var(--adm-muted);border:1px solid var(--adm-border);border-radius:999px;padding:5px 7px;background:var(--adm-bg)}.big-integration-card.connected .state{color:var(--adm-accent);border-color:color-mix(in srgb,var(--adm-accent) 35%,var(--adm-border))}.integration-card-copy{padding-top:13px}.big-integration-card h3{font-size:15px;margin:0;color:var(--adm-text)}.big-integration-card p{font-size:11px;line-height:1.48;color:var(--adm-muted);margin:7px 0 0}.integration-card-footer{margin-top:auto;padding-top:13px;border-top:1px solid var(--adm-border);display:flex;align-items:center;justify-content:space-between;gap:10px}.integration-card-footer small{min-width:0;font-size:12px;color:var(--adm-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.big-integration-card .open-label{padding:0;font-size:12px;font-weight:900;color:var(--adm-accent);white-space:nowrap}.big-integration-card .open-label b{font-size:13px}
    .modal-backdrop{position:fixed;inset:0;z-index:1200;background:rgba(0,0,0,.48);display:flex;align-items:center;justify-content:center;padding:16px}.integration-modal{width:min(760px,100%);max-height:calc(100vh - 32px);overflow:auto;background:var(--adm-surface);border:1px solid var(--adm-border);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.28)}.modal-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;gap:12px;align-items:flex-start;padding:16px 18px;border-bottom:1px solid var(--adm-border);background:var(--adm-surface)}.modal-head h2{margin:0;font-size:20px}.modal-head span{font-size:11px;color:var(--adm-muted)}.modal-close{padding:7px 10px;font-size:18px;line-height:1}.modal-body{padding:18px}.modal-actions{position:sticky;bottom:-18px;display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap;margin:20px -18px -18px;padding:14px 18px;border-top:1px solid var(--adm-border);background:var(--adm-surface)}.modal-actions .primary{border-color:var(--adm-accent);color:var(--adm-accent);font-weight:800}.modal-actions .danger{color:var(--adm-red)}.warning-box{padding:12px;border-radius:10px;border:1px solid #f59e0b55;background:var(--adm-amber)12;margin-bottom:14px;font-size:13px}.warning-box div{margin-top:4px;color:var(--adm-muted)}
    ...
    .ai-runtime-box{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px;border:1px solid var(--adm-border);background:var(--adm-surface2);border-radius:12px;margin-bottom:14px}.ai-runtime-box>div{min-width:0;display:grid;gap:3px}.ai-runtime-box span{font-size:11px;color:var(--adm-muted)}.ai-runtime-pill{flex:none;padding:5px 8px;border-radius:999px;font-size:12px!important;font-weight:900;letter-spacing:.06em}.ai-runtime-pill.on{color:var(--adm-accent);border:1px solid color-mix(in srgb,var(--adm-accent) 40%,var(--adm-border))}.ai-runtime-pill.off{border:1px solid var(--adm-border)}
    ...
    .ai-toggle-row{display:flex;gap:10px;align-items:flex-start;padding:13px;border:1px solid var(--adm-border);border-radius:10px;margin:10px 0}.ai-toggle-row input{width:auto;margin-top:2px}.ai-toggle-row span{display:grid;gap:3px}.ai-toggle-row small{font-size:11px;font-weight:400;color:var(--adm-muted);line-height:1.45}.ai-toggle-row.disabled{opacity:.55}.ai-review-callout{display:grid;gap:4px;margin-top:14px}.ai-privacy{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:12px;border:1px solid var(--adm-border);border-radius:10px;background:var(--adm-surface2);margin-top:12px}.ai-privacy>b{grid-column:1/-1;font-size:12px}.ai-privacy label{display:flex;align-items:center;gap:7px;font-size:11px;color:var(--adm-muted)}.ai-privacy input{width:auto}.ai-runtime-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:10px 0 14px}.ai-model-cap-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:10px 0}.ai-model-cap-list button{display:grid;gap:3px;text-align:left;padding:9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-bg);color:var(--adm-text);min-width:0}.ai-model-cap-list button.selected{border-color:var(--adm-accent);background:color-mix(in srgb,var(--adm-accent) 7%,var(--adm-surface))}.ai-model-cap-list b{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ai-model-cap-list span,.ai-model-selected span{font-size:11px;line-height:1.35;color:var(--adm-muted)}.ai-model-selected{display:grid;gap:4px;padding:9px 10px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2);margin-bottom:10px}.ai-model-selected b{font-size:12px}
    .integration-diagnostic-list{display:grid;gap:8px}.integration-diagnostic-list>div{display:grid;grid-template-columns:30px minmax(0,1fr);gap:9px;align-items:center;padding:10px;border:1px solid var(--adm-border);border-radius:11px;background:var(--adm-surface2)}.integration-diagnostic-list>div>span{width:30px;height:30px;border-radius:9px;display:grid;place-items:center;font-weight:950;background:var(--adm-surface)}.integration-diagnostic-list>div.ok>span{color:var(--adm-success)}.integration-diagnostic-list>div.warn>span{color:var(--adm-amber)}.integration-diagnostic-list>div>div{display:grid;gap:2px}.integration-diagnostic-list b{font-size:11px}.integration-diagnostic-list small{font-size:12px;line-height:1.4;color:var(--adm-muted)}
    .system-secret-panel{display:grid;gap:10px;padding:14px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-surface);margin-bottom:12px}.system-secret-panel>div:first-child{display:grid;gap:3px}.system-secret-panel>div:first-child span{font-size:12px;color:var(--adm-muted)}.system-secret-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.system-secret-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px;padding:9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-bg);min-width:0}.system-secret-row>div:first-child{display:grid;gap:2px;min-width:0}.system-secret-row b{font-size:12px}.system-secret-row small{font-size:11px;color:var(--adm-muted)}.system-secret-row code{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.system-secret-row>div:last-child{display:flex;gap:4px;align-items:center}.system-secret-row button{padding:6px;font-size:11px}
    .credential-inspector{display:grid;gap:8px;padding:11px;border:1px solid var(--adm-border);border-radius:11px;background:var(--adm-surface2);margin:10px 0 14px;min-width:0}.credential-inspector-head{display:grid;gap:2px}.credential-inspector-head b{font-size:11px}.credential-inspector-head span{font-size:12px;color:var(--adm-muted)}.credential-secret-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center;padding-top:7px;border-top:1px solid var(--adm-border);min-width:0}.credential-secret-row>div:first-child{display:grid;gap:3px;min-width:0}.credential-secret-row small{font-size:11px;color:var(--adm-muted)}.credential-origin{display:block;font-size:11px;color:var(--adm-muted);overflow-wrap:anywhere}.credential-protected{font-size:12px;color:var(--adm-muted);font-weight:800}.credential-secret-row code{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}.credential-secret-row>div:last-child{display:flex;gap:5px}.credential-secret-row button{padding:6px 8px;font-size:12px}
    @media(max-width:900px){.integration-card-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}

    .ai-diagnostic-hero{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:12px;align-items:center;padding:14px;border:1px solid var(--adm-border);border-radius:15px;background:linear-gradient(145deg,var(--adm-surface2),var(--adm-bg));margin-bottom:12px}.ai-diagnostic-hero-icon{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;font-weight:950;border:1px solid var(--adm-border)}.ai-diagnostic-hero.ok .ai-diagnostic-hero-icon{color:var(--adm-success);border-color:var(--adm-success)55;background:#16a34a0d}.ai-diagnostic-hero.warn .ai-diagnostic-hero-icon{color:var(--adm-amber);border-color:#f59e0b55;background:var(--adm-amber)0d}.ai-diagnostic-hero small,.ai-usage-head small{font-size:11px;font-weight:950;letter-spacing:.12em;color:var(--adm-muted)}.ai-diagnostic-hero h3{font-size:15px;margin:2px 0}.ai-diagnostic-hero p{margin:0;font-size:11px;color:var(--adm-muted);line-height:1.4}.ai-diagnostic-hero>span{font-size:12px;font-weight:900;border:1px solid var(--adm-border);border-radius:999px;padding:5px 7px;color:var(--adm-muted)}.ai-runtime-metrics-top{margin-bottom:12px}.ai-diagnostic-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.ai-diagnostic-provider{position:relative;display:flex;flex-direction:column;gap:9px;padding:12px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-surface2);overflow:hidden}.ai-diagnostic-provider:before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--adm-border)}.ai-diagnostic-provider.provider-ok:before{background:#16a34a}.ai-diagnostic-provider.provider-warn:before{background:var(--adm-amber)}.ai-provider-head{display:grid!important;grid-template-columns:auto minmax(0,1fr) auto;gap:8px!important;align-items:center}.ai-provider-mark{width:30px;height:30px;border-radius:9px;display:grid!important;place-items:center;border:1px solid var(--adm-border);background:var(--adm-bg);font-size:12px;font-weight:950}.ai-provider-head>div:nth-child(2){display:grid;gap:2px}.ai-provider-head b{font-size:12px}.ai-provider-head span{font-size:12px;color:var(--adm-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ai-provider-head strong{font-size:11px;letter-spacing:.06em;color:var(--adm-muted)}.provider-ok .ai-provider-head strong{color:var(--adm-success)}.provider-warn .ai-provider-head strong{color:var(--adm-amber)}.ai-diagnostic-provider p{margin:0;color:var(--adm-muted);font-size:12px;line-height:1.45;min-height:30px}.ai-diagnostic-tests{display:flex!important;flex-wrap:wrap;gap:5px!important}.ai-diagnostic-tests span{padding:4px 6px;border:1px solid var(--adm-border);border-radius:999px;font-size:11px!important;color:var(--adm-muted)}.ai-provider-open{margin-top:auto;padding:7px 9px!important;font-size:12px!important;font-weight:850!important;text-align:left}.ai-usage-panel{margin-top:13px;padding:12px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-bg)}.ai-usage-head{display:flex;align-items:end;justify-content:space-between;gap:8px;margin-bottom:9px}.ai-usage-head>div{display:grid;gap:2px}.ai-usage-head b{font-size:12px}.ai-usage-head>span{font-size:12px;color:var(--adm-muted)}.ai-diagnostic-note{margin-top:12px!important;padding:10px;border:1px dashed var(--adm-border);border-radius:10px;background:var(--adm-bg)}
    @media(max-width:620px){.system-secret-grid{grid-template-columns:1fr}.system-secret-row{grid-template-columns:1fr}.system-secret-row>div:last-child{display:grid;grid-template-columns:1fr 1fr}.system-secret-row code{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all}.credential-secret-row{grid-template-columns:1fr}.credential-secret-row>div:last-child{display:grid;grid-template-columns:1fr 1fr}.credential-secret-row code{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all}.integrations-titlebar>button,.hub-heading>button{width:100%}.utility-strip{align-items:stretch;flex-direction:column}.utility-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))}.utility-actions button{padding:9px 5px;font-size:11px}.integration-hub{padding:12px}.integration-card-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.big-integration-card{min-height:164px;padding:12px}.integration-card-icon{width:34px;height:34px;border-radius:10px}.big-integration-card h3{font-size:12px}.big-integration-card p{font-size:12px;line-height:1.4}.integration-card-footer{align-items:flex-end;gap:5px}.integration-card-footer small{font-size:11px;max-width:70%}.big-integration-card .open-label{font-size:12px}.modal-backdrop{padding:12px;align-items:center;justify-content:center}.integration-modal{border-radius:18px;max-height:calc(100dvh - 24px);width:min(680px,100%)}.modal-body{padding:15px}.modal-actions{margin:18px -15px -15px;padding:12px 15px;display:grid;grid-template-columns:1fr 1fr}.modal-actions button{width:100%}.choice-grid{grid-template-columns:1fr}.integration-status-grid,.github-diagnostic-grid{grid-template-columns:1fr}.integration-help-links{display:grid!important;grid-template-columns:1fr}.integration-help-links a{text-align:center}.ai-steps{grid-template-columns:repeat(2,minmax(0,1fr))!important}.ai-params-grid{grid-template-columns:1fr!important}.ai-privacy,.ai-runtime-metrics{grid-template-columns:1fr 1fr!important}.ai-model-cap-list{grid-template-columns:1fr!important}.ai-diagnostic-list{grid-template-columns:1fr}.ai-diagnostic-hero{grid-template-columns:auto minmax(0,1fr)}.ai-diagnostic-hero>span{grid-column:1/-1;width:max-content}.model-picker-head{align-items:flex-start;flex-direction:column}.model-picker-head button{width:100%}.ai-steps button{font-size:11px;padding:9px 5px!important}}
  `}</style>
 </div>
}
...
const INTEGRATION_ICONS={mongodb:'DB',cloudinary:'☁',cloudflare:'CF',github:'GH',render:'R',vercel:'▲',gemini:'✦',openrouter:'OR',api_ninjas:'✧',api_football:'⚽',resend:'✉'}
function IntegrationCard({id,name,description,status,onOpen}){
 const configured=Boolean(status?.configured),isAi=AI_PROVIDERS.includes(id),enabled=status?.metadata?.enabled!==false
 const state=!configured?'PENDENTE':isAi&&!enabled?'PAUSADO':isAi&&status?.metadata?.primary?'PRINCIPAL':isAi?'ATIVO':'CONFIGURADO'
 const detail=isAi&&configured?`Modelo · ${normalizeAiModel(id,status?.metadata?.model)}`:configured?(status?.metadata?.identity?.label||status?.identity?.label||'Credencial protegida no cofre'):'Toque para configurar'
 return <button className={`big-integration-card ${configured?'connected':''} ${isAi&&!enabled?'paused':''}`} onClick={()=>onOpen(id)}>
  <div className="integration-card-accent"/>
  <div className="card-top"><span className="integration-card-icon">{INTEGRATION_ICONS[id]||'•'}</span><span className="state">{state}</span></div>
  <div className="integration-card-copy"><h3>{name}</h3><p>{description}</p></div>
  <div className="integration-card-footer"><small>{detail}</small><span className="open-label">Abrir <b>→</b></span></div>
 </button>
}
function Modal({title,subtitle,onClose,children}){return <DSModal open onClose={onClose} title={title} size="lg"><div className="integration-modal-subtitle">{subtitle}</div>{children}</DSModal>}
...
function AIWizard({provider,form,setForm,current}){
 ...
 return <div>
  ...
  {step===4&&<div><h3 style={{margin:'0 0 12px'}}>Revisão da configuração</h3><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:9}}><StatusBox label="Provedor" value={name}/><StatusBox label="Estado" value={active?'Ativo':'Pausado'}/><StatusBox label="Modelo" value={normalizeAiModel(provider,form.metadata.model)}/><StatusBox label="Ordem" value={active?(form.metadata.primary?'Principal':'Fallback'):'Fora da rotação'}/></div><div className="ai-review-callout"><b>Teste recomendado antes de salvar em produção</b><span>O teste faz uma pequena geração real. Em uso normal, se a IA principal falhar, a outra integração ativa é usada automaticamente.</span></div></div>}
...
function StatusBox({label,value}){return <div style={{padding:10,borderRadius:9,border:'1px solid var(--adm-border)',background:'var(--adm-bg)',minWidth:0}}><div style={{fontSize:12,color:'var(--adm-muted)',fontWeight:800}}>{label.toUpperCase()}</div><div style={{marginTop:3,fontSize:13,fontWeight:700,overflowWrap:'anywhere'}}>{value}</div></div>}
...
function PortalApiConnector({provider,current,form,setForm}){
 ...
 return <div>
  <div style={{padding:12,borderRadius:10,border:'1px solid var(--adm-border)',background:'var(--adm-surface2)',marginBottom:14,fontSize:12,lineHeight:1.55}}>
   ...
   <div style={{color:'var(--adm-muted)',marginTop:5}}>{football?'A chave não é enviada automaticamente ao navegador; só pode ser recuperada individualmente por administrador autenticado. Live scores usam cache curto; jogos do dia usam cache maior para economizar cota.':'A chave não é enviada automaticamente ao navegador; só pode ser recuperada individualmente por administrador autenticado. Cada signo é consultado sob demanda e fica em cache por várias horas.'}</div>
  </div>
 ...
 return <div>
  ...
  {cloudflareTab==='api'?<>
   ...
   <div style={{padding:11,border:'1px solid var(--adm-border)',borderRadius:9,background:'var(--adm-bg)',margin:'3px 0 17px'}}>
    <div style={{fontSize:12,fontWeight:800,color:'var(--adm-muted)'}}>ENDPOINT R2 GERADO AUTOMATICAMENTE</div>
    <code style={{display:'block',fontSize:12,marginTop:5,color:'var(--adm-text)',overflowWrap:'anywhere'}}>{endpoint}</code>
   </div>
  ...
  {diagnostic&&<div style={{marginTop:14,padding:12,borderRadius:10,border:`1px solid ${diagnostic.ok===false?'color-mix(in srgb,var(--adm-red) 28%,var(--adm-border))':'color-mix(in srgb,var(--adm-success) 28%,var(--adm-border))'}`,background:diagnostic.ok===false?'color-mix(in srgb,var(--adm-red) 7%,var(--adm-surface))':'color-mix(in srgb,var(--adm-success) 7%,var(--adm-surface))'}}>
   ...
   {diagnostic.erro&&<div style={{fontSize:11,color:'var(--adm-red)',marginTop:5}}>{diagnostic.erro}</div>}
   ...
   {diagnostic.endpointS3&&<div style={{fontSize:12,color:'var(--adm-muted)',marginTop:4,overflowWrap:'anywhere'}}>S3: <code>{diagnostic.endpointS3}</code></div>}
   {diagnostic.s3?.configured&&<div style={{fontSize:11,color:diagnostic.s3.ok?'var(--adm-success)':'var(--adm-amber)',marginTop:5}}>{diagnostic.s3.ok?`R2 S3 válido · ${diagnostic.s3.buckets?.length||0} bucket(s) acessível(is)`:`R2 S3 não validado: ${diagnostic.s3.error||'revise as credenciais'}`}</div>}
  </div>}
...
function SystemSecretRow({item}){
 const [value,setValue]=useState(''),[busy,setBusy]=useState(false)
 const authorize=async()=>!item.critical||await confirmAction(`Revelar ${item.name}? Este é um segredo especialmente sensível do backend. Exponha-o somente em um dispositivo confiável.`)
 const reveal=async()=>{if(value){setValue('');return}if(!await authorize())return;setBusy(true);try{const d=await API('/system-secret/reveal',{method:'POST',body:JSON.stringify({name:item.name})});setValue(d.value||'')}catch(e){toast.error(e.message)}finally{setBusy(false)}}
 const copy=async()=>{let v=value;if(!v){if(!await authorize())return;setBusy(true);try{const d=await API('/system-secret/reveal',{method:'POST',body:JSON.stringify({name:item.name})});v=d.value||''}catch(e){toast.error(e.message);return}finally{setBusy(false)}}if(v){await navigator.clipboard.writeText(v);toast.success('Copiado')}}
 useEffect(()=>()=>setValue(''),[])
 return <div className="system-secret-row"><div><b>{item.name}</b><small>Origem: {item.source||'não identificada'} · {item.configured?'Configurada':'Não configurada'}{item.critical?' · sensível':''}</small><code>{value||item.masked||'—'}</code>{item.protectedReason&&<small>{item.protectedReason}</small>}</div><div>{item.configured&&item.revealable!==false&&<><button onClick={reveal} disabled={busy}>{busy?'…':value?'Ocultar':'Visualizar'}</button><button onClick={copy} disabled={busy}>Copiar</button></>}</div></div>
}

function StoredCredentialInspector({id,current}){
 const [revealed,setRevealed]=useState({}),[loading,setLoading]=useState('')
 const fields=id==='cloudflare'?[['secret','CF_API_TOKEN'],['r2AccessKeyId','CF_R2_ACCESS_KEY_ID'],['r2SecretAccessKey','CF_R2_SECRET_ACCESS_KEY']]:id==='cloudinary'?[['secret','CLOUDINARY_API_SECRET'],['apiKey','CLOUDINARY_API_KEY']]:[["secret",id==='mongodb'?'MONGO_URI':({github:'GITHUB_TOKEN',render:'RENDER_API_KEY',vercel:'VERCEL_TOKEN',gemini:'GEMINI_API_KEY',openrouter:'OPENROUTER_API_KEY',api_ninjas:'API_NINJAS_KEY',api_football:'API_FOOTBALL_KEY',resend:'RESEND_API_KEY'}[id]||'CREDENCIAL')]]
 const fieldInfo=field=>id==='mongodb'?{configured:current?.configured,revealable:current?.configured,masked:current?.configured?'••••••••••••••••':'',source:current?.source}:current?.fields?.[field]
 const reveal=async(field)=>{if(revealed[field]){setRevealed(x=>({...x,[field]:''}));return}const info=fieldInfo(field);if(!info?.configured)return toast.error(`${info?.label||'Credencial'} não configurada.`);if(info?.revealable===false)return toast.error(info?.locked?'Credencial bloqueada pela chave de outra instalação.':'Este valor não pode ser recuperado nesta instalação.');setLoading(field);try{const d=await API(id==='mongodb'?'/mongodb/reveal':`/${id}/reveal`,{method:'POST',body:JSON.stringify({field})});setRevealed(x=>({...x,[field]:d.value||''}))}catch(e){toast.error(e.message)}finally{setLoading('')}}
 const copy=async(field)=>{let value=revealed[field];const info=fieldInfo(field);if(!info?.configured)return toast.error(`${info?.label||'Credencial'} não configurada.`);if(info?.revealable===false)return toast.error(info?.locked?'Credencial bloqueada pela chave de outra instalação.':'Este valor não pode ser recuperado nesta instalação.');if(!value){setLoading(field);try{const d=await API(id==='mongodb'?'/mongodb/reveal':`/${id}/reveal`,{method:'POST',body:JSON.stringify({field})});value=d.value||''}catch(e){toast.error(e.message);return}finally{setLoading('')}}if(value){await navigator.clipboard.writeText(value);toast.success('Copiado')}}
 useEffect(()=>()=>setRevealed({}),[])
 return <div className="credential-inspector"><div className="credential-inspector-head"><b>Credencial cadastrada</b><span>O valor completo só é solicitado ao backend para o campo que você escolher.</span></div>{fields.map(([field,label])=>{const info=fieldInfo(field)||{};return <div className="credential-secret-row" key={field}><div><small>{label}</small><span className="credential-origin">Origem: {info.source||'não identificada'} · {info.locked?'Bloqueada':info.unknown?'Estado desconhecido':info.configured?'Configurada':'Não configurada'}</span><code>{revealed[field]||info.masked||'—'}</code></div><div>{info.configured&&info.revealable!==false&&<><button type="button" onClick={()=>reveal(field)} disabled={loading===field}>{loading===field?'…':revealed[field]?'Ocultar':'Visualizar'}</button><button type="button" onClick={()=>copy(field)} disabled={loading===field}>Copiar</button></>}{info.configured&&info.revealable===false&&<span className="credential-protected">Protegida</span>}</div></div>})}</div>
}

function SecretField({label,onChange,value='',...props}){const [show,setShow]=useState(false);useEffect(()=>{if(!value)setShow(false)},[value]);return <label style={{display:'block',marginBottom:13,fontSize:13,fontWeight:600}}>{label}<div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) auto',gap:8,marginTop:6}}><input {...props} value={value} type={show?'text':'password'} onChange={e=>onChange(e.target.value)} style={{marginTop:0,minWidth:0}} autoComplete="new-password"/><button type="button" onClick={()=>setShow(v=>!v)} disabled={!value} title={value?(show?'Ocultar o valor novo digitado':'Visualizar o valor novo digitado'):'A credencial salva é visualizada no bloco “Credencial cadastrada” acima'}>{show?'Ocultar digitado':'Ver digitado'}</button></div></label>}
