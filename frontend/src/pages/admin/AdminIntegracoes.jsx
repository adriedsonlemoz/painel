...
function SystemSecretRow({item}){
 ...
 const reveal=async()=>{if(value){setValue('');return}setBusy(true);try{const d=await API('/system-secret/reveal',{method:'POST',body:JSON.stringify({name:item.name})});setValue(d.value||'')}catch(e){toast.error(e.message)}finally{setBusy(false)}}
 const copy=async()=>{let v=value;if(!v){setBusy(true);try{const d=await API('/system-secret/reveal',{method:'POST',body:JSON.stringify({name:item.name})});v=d.value||'';setValue(v)}catch(e){toast.error(e.message);return}finally{setBusy(false)}}if(v){await navigator.clipboard.writeText(v);toast.success('Copiado')}}
 useEffect(()=>()=>setValue(''),[])
 return <div className="system-secret-row"><div><b>{item.name}</b><small>Origem: variável de ambiente · {item.configured?'Configurada':'Não configurada'}</small><code>{value||item.masked||'—'}</code></div><div>{item.configured&&<><button onClick={reveal} disabled={busy}>{busy?'…':value?'Ocultar':'Visualizar'}</button><button onClick={copy} disabled={busy}>Copiar</button></>}</div></div>
}
...
function StoredCredentialInspector({id,current}){
 ...
 const reveal=async(field)=>{if(revealed[field]){setRevealed(x=>({...x,[field]:''}));return}setLoading(field);try{const d=await API(id==='mongodb'?'/mongodb/reveal':`/${id}/reveal`,{method:'POST',body:JSON.stringify({field})});setRevealed(x=>({...x,[field]:d.value||''}))}catch(e){toast.error(e.message)}finally{setLoading('')}}
 const copy=async(field)=>{let value=revealed[field];if(!value){setLoading(field);try{const d=await API(id==='mongodb'?'/mongodb/reveal':`/${id}/reveal`,{method:'POST',body:JSON.stringify({field})});value=d.value||'';setRevealed(x=>({...x,[field]:value}))}catch(e){toast.error(e.message);return}finally{setLoading('')}}if(value){await navigator.clipboard.writeText(value);toast.success('Copiado')}}
 useEffect(()=>()=>setRevealed({}),[])
 return <div className="credential-inspector"><div className="credential-inspector-head"><b>Credencial cadastrada</b><span>Origem: {current?.source||'backend'} · valor solicitado somente ao tocar em visualizar.</span></div>{fields.map(([field,label])=><div className="credential-secret-row" key={field}><div><small>{label}</small><code>{revealed[field]||current?.masked||'••••••••••••••••'}</code></div><div><button type="button" onClick={()=>reveal(field)} disabled={loading===field}>{loading===field?'…':revealed[field]?'Ocultar':'Visualizar'}</button><button type="button" onClick={()=>copy(field)} disabled={loading===field}>Copiar</button></div></div>)}</div>
}
