...
export default function AdminVariaveisAmbiente() {
  ...
  async function remove(item) {
    ...
    try {
      ...
      toast.success(r.mensagem || 'Variável removida.'); await loadEnv()
    } catch (e) { toast.error(e.message) }
    finally { setBusy(false) }
  }
  ...
  return <div className="adm-page ev-page">
    ...
    <section className="ev-hero"><div><small>CONFIGURAÇÃO CENTRAL</small><h1>Variáveis de ambiente</h1><p>Gerencie Vercel e Render pela API sem abrir os painéis externos. Valores completos nunca são exibidos depois de salvos.</p></div><button onClick={loadEnv}><AdminIcon name="refresh" size={13}/> Atualizar</button></section>
    ...
    <section className="ev-card">
      ...
      {loadingEnv?<div className="ev-empty">Consultando variáveis…</div>:items.length?<div className="ev-list">{items.map((x,i)=><div className="ev-row" key={`${x.id||i}-${x.key}`}><div><b>{x.key}</b><small>{provider==='vercel'?`${(x.target||[]).join(', ')||'production'}${x.type?` · ${x.type}`:''}`:'Render · variável direta do serviço'}</small></div><code>{x.valueMasked||'protegida'}</code><div><button onClick={()=>edit(x)} disabled={busy}>Editar</button><button className="danger" onClick={()=>remove(x)} disabled={busy}>Excluir</button></div></div>)}</div>:<div className="ev-empty">Nenhuma variável encontrada neste recurso.</div>}
    </section>
