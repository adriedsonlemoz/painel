...
export default function AdminVariaveisAmbiente() {
  ...
  async function reveal(item) {
    ...
    if(item.revealable===false) return toast.error(item.providerLimitation||'Este provedor não permite recuperar o valor original desta variável.')
    setRevealBusy(rk)
    try {
  ...
  async function copyValue(item) {
    ...
    if(item.revealable===false) return toast.error(item.providerLimitation||'Este provedor não permite recuperar o valor original desta variável.')
    if(!v){ setRevealBusy(rk); try{const r=provider==='vercel'?await infraestruturaService.vercelRevelarVariavel(resourceId,item.key,item.id||''):await infraestruturaService.renderRevelarVariavel(resourceId,item.key);v=r.value||''}catch(e){toast.error(e.message);return}finally{setRevealBusy('')} }
  ...
  return <div className="adm-page ev-page">
    ...
    <section className="ev-card">
      ...
      <div className="ev-security">🔒 A listagem recebe somente máscara e metadados. O valor completo de uma variável recuperável é buscado individualmente somente ao tocar em Visualizar ou Copiar.</div>
      {loadingEnv?<div className="ev-empty">Consultando variáveis…</div>:items.length?<div className="ev-list">{items.map((x,i)=>{const rk=x.id||x.key;const rv=revealed[rk];const availability=x.revealable===false?'valor protegido pelo provedor':x.revealable===true?'valor recuperável sob demanda':'recuperação não confirmada';return <div className="ev-row" key={`${x.id||i}-${x.key}`}><div><b>{x.key}</b><small>{provider==='vercel'?`${(x.target||[]).join(', ')||'production'}${x.type?` · ${x.type}`:''}`:'Render · variável direta do serviço'} · Origem: {x.origin||provider} · Status: {x.status==='configured'?'configurada':'não testada'} · {availability}{x.updatedAt?` · Atualizada: ${new Date(x.updatedAt).toLocaleString('pt-BR')}`:''}</small>{x.providerLimitation&&<small className="ev-limitation">{x.providerLimitation}</small>}</div><code className={rv?'revealed':''}>{rv||x.valueMasked||'protegida'}</code><div>{x.revealable!==false&&<><button onClick={()=>reveal(x)} disabled={busy||revealBusy===rk}>{revealBusy===rk?'…':rv?'Ocultar':'Visualizar'}</button><button onClick={()=>copyValue(x)} disabled={busy||revealBusy===rk}>Copiar</button></>}<button onClick={()=>edit(x)} disabled={busy}>Editar</button><button className="danger" onClick={()=>setConfirmDelete(x)} disabled={busy}>Excluir</button></div></div>})}</div>:<div className="ev-empty">Nenhuma variável encontrada neste recurso.</div>}
    </section>
    ...
    <style>{`
      .ev-page{display:grid;gap:12px}.ev-loading{min-height:220px;display:flex;align-items:center;justify-content:center;gap:7px;color:var(--adm-muted)}.ev-breadcrumb{display:flex;gap:6px;font-size:12px;color:var(--adm-muted)}.ev-breadcrumb a{color:var(--adm-accent);text-decoration:none}.ev-hero{display:flex;justify-content:space-between;align-items:flex-end;gap:15px;padding:17px;border:1px solid var(--adm-border);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--adm-accent) 6%,var(--adm-surface)),var(--adm-surface))}.ev-hero small,.ev-card header small{font-size:11px;font-weight:900;letter-spacing:.13em;color:var(--adm-accent)}.ev-hero h1{margin:4px 0 5px;font-size:24px}.ev-hero p{margin:0;max-width:720px;font-size:12px;color:var(--adm-muted);line-height:1.5}.ev-hero button,.ev-card button{border:1px solid var(--adm-border);border-radius:8px;padding:8px 10px;background:var(--adm-surface2);color:var(--adm-text);font-size:12px;font-weight:800}.ev-card{padding:13px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-surface)}.ev-provider-tabs{display:flex;gap:6px;margin-bottom:10px}.ev-provider-tabs button.active{background:#111;color:#fff}.ev-provider-tabs button.render.active{background:#7c3aed;color:#fff}.ev-resource{display:grid;gap:5px;font-size:12px;font-weight:800}.ev-resource select,.ev-fields input{width:100%;box-sizing:border-box;padding:9px;border:1px solid var(--adm-border);border-radius:8px;background:var(--adm-bg);color:var(--adm-text)}.ev-primary,.ev-security{margin-top:8px;padding:8px 9px;border-radius:8px;font-size:11px}.ev-primary{background:color-mix(in srgb,var(--adm-success) 6%,transparent);color:var(--adm-success)}.ev-security{background:var(--adm-surface2);color:var(--adm-muted)}.ev-card>header{display:flex;justify-content:space-between;gap:8px;align-items:flex-end}.ev-card h2{font-size:15px;margin:3px 0 0}.ev-card header>span{font-size:11px;color:var(--adm-muted)}.ev-list{display:grid;margin-top:9px}.ev-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:9px;align-items:center;padding:9px 2px;border-top:1px solid var(--adm-border)}.ev-row>div:first-child{display:grid;gap:2px;min-width:0}.ev-row b{font-size:12px;overflow-wrap:anywhere}.ev-row small,.ev-row code{font-size:11px;color:var(--adm-muted);min-width:0}.ev-limitation{color:var(--adm-amber,#d97706)!important;overflow-wrap:anywhere}.ev-row code{max-width:min(42vw,360px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ev-row code.revealed{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;color:var(--adm-text)}.ev-row>div:last-child{display:flex;gap:4px;flex-wrap:wrap}.ev-row .danger{color:var(--adm-red)}.ev-empty{padding:24px;text-align:center;color:var(--adm-muted);font-size:12px}.ev-editor{display:grid;gap:10px}.ev-editor .link{border:0;background:transparent;color:var(--adm-accent)}.ev-fields{display:grid;grid-template-columns:.85fr 1.15fr;gap:8px}.ev-fields label{display:grid;gap:5px;font-size:12px;font-weight:800}.ev-options{display:flex;gap:9px;align-items:center;flex-wrap:wrap;font-size:11px;color:var(--adm-muted)}.ev-options label{display:flex;gap:3px;align-items:center}.ev-actions{display:flex;justify-content:flex-end;gap:7px}.ev-actions .primary{background:var(--adm-accent);border-color:var(--adm-accent);color:#fff}.ev-actions button:disabled{opacity:.5}.ev-editor>p{margin:0;font-size:11px;color:var(--adm-muted)}
