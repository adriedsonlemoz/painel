...
export default function AdminProjetoPlataforma() {
  ...
  async function openEnv(kind) {
    ...
    if (!target) return
    setEnv({ kind, resourceId: target.id, loading: true, items: [], title: `Variáveis · ${target.nome}` })
    try {
      const r = kind === 'render'
  ...
  async function renderAction(type, payload = '', clearCache = false) {
    ...
    if (!rd) return
    if (type === 'rollback' && !await confirmAction('Fazer rollback do serviço Render para este deploy?')) return
    if (type === 'cancel' && !await confirmAction('Cancelar este deploy em andamento?')) return
    setBusy(true)
    try {
  ...
  async function vercelAction(type, deploymentId = '') {
    ...
    if (type === 'cancel' && !await confirmAction('Cancelar este deployment da Vercel?')) return
    setBusy(true)
    try {
      const r = type === 'cancel'
  ...
  async function saveRenderEnv(key, value, deployAfter = false) {
    ...
    setBusy(true)
    try {
      const r = await infraestruturaService.renderSalvarVariavel(rd.id, key, value, { deploy: deployAfter })
      ...
      if (deployAfter) await load()
    } catch (e) { toast.error(e.message) }
    finally { setBusy(false) }
  }

  async function saveVercelEnv(key, value, deployAfter = false, options = {}) {
    const vc = data?.provedores?.vercel
    if (!vc) return
    setBusy(true)
    try {
      const r = await infraestruturaService.vercelSalvarVariavel(vc.id, key, value, {
        envId: options.envId || '', target: options.target || ['production'], sensitive: Boolean(options.sensitive), deploy: deployAfter,
      })
      toast.success(r.mensagem || 'Variável salva na Vercel.')
      const vars = await infraestruturaService.vercelVariaveis(vc.id)
      setEnv(e => ({ ...e, items: vars.env || [] }))
      if (deployAfter) await load()
    } catch (e) { toast.error(e.message) }
    finally { setBusy(false) }
  ...
  async function removeEnv(item) {
    ...
    if (!await confirmAction(`Remover a variável “${item.key}” da ${env.kind === 'vercel' ? 'Vercel' : 'Render'}?`)) return
    const target = env.kind === 'render' ? data?.provedores?.render : data?.provedores?.vercel
    if (!target) return
    setBusy(true)
    try {
      const r = env.kind === 'render'
        ? await infraestruturaService.renderRemoverVariavel(target.id, item.key)
        : await infraestruturaService.vercelRemoverVariavel(target.id, item.key, item.id || '')
      toast.success(r.mensagem || 'Variável removida.')
      const vars = env.kind === 'render'
        ? await infraestruturaService.renderVariaveis(target.id)
        : await infraestruturaService.vercelVariaveis(target.id)
      setEnv(e => ({ ...e, items: vars.env || [] }))
    } catch (e) { toast.error(e.message) }
    ...
    <div className="adm-page pd-page">
      ...
      {tab === 'variaveis' && (
        <section className="pd-box">
          ...
          {env && <EnvPanel env={env} onClose={() => setEnv(null)} onSave={env.kind === 'render' ? saveRenderEnv : saveVercelEnv} onDelete={removeEnv} busy={busy} />}
        </section>
      )}
      ...
      <style>{`
        .pd-page{display:grid;gap:13px}.pd-loading{min-height:220px;display:flex;justify-content:center;align-items:center;gap:8px;color:var(--adm-muted);font-size:11px}.pd-breadcrumb{display:flex;gap:6px;align-items:center;font-size:12px;color:var(--adm-muted)}.pd-breadcrumb a{display:flex;align-items:center;gap:2px;color:var(--adm-accent);text-decoration:none}.pd-hero{display:flex;justify-content:space-between;align-items:flex-end;gap:14px;padding:17px;border:1px solid var(--adm-border);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--adm-accent) 5%,var(--adm-surface)),var(--adm-surface))}.pd-kicker,.pd-box header small,.pd-panel-health header small,.pd-incident small{font-size:11px;font-weight:900;letter-spacing:.13em;color:var(--adm-accent)}.pd-hero h1{margin:4px 0 7px;font-size:24px;color:var(--adm-text)}.pd-meta{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.pd-meta>span:not(.pd-pill){font-size:11px;color:var(--adm-muted);border:1px solid var(--adm-border);border-radius:999px;padding:4px 6px}.pd-actions{display:flex;gap:6px;flex-wrap:wrap}.pd-actions a,.pd-actions button,.pd-load-actions button,.pd-log-shortcuts button,.pd-provider footer button,.pd-incident button,.pd-log-tools button{display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:7px 9px;border:1px solid var(--adm-border);border-radius:8px;background:var(--adm-surface2);color:var(--adm-text);text-decoration:none;font-size:12px;font-weight:750;cursor:pointer}.pd-actions button:disabled,.pd-provider button:disabled,.pd-deploy-actions button:disabled,.pd-log-tools button:disabled{opacity:.55;cursor:not-allowed}.pd-tabs{display:flex;gap:5px;overflow:auto}.pd-tabs button{white-space:nowrap;padding:7px 10px;border:1px solid var(--adm-border);border-radius:999px;background:var(--adm-surface);color:var(--adm-muted);font-size:12px;font-weight:800}.pd-tabs button.active{color:#fff;background:var(--adm-accent);border-color:var(--adm-accent)}.pd-stack{display:grid;gap:11px}.pd-summary,.pd-health-grid,.pd-analysis-meta{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px}.pd-summary>div,.pd-health-grid>div,.pd-analysis-meta>div{display:grid;gap:2px;padding:10px 11px;border:1px solid var(--adm-border);border-radius:11px;background:var(--adm-surface)}.pd-summary b,.pd-health-grid b{font-size:16px;color:var(--adm-text)}.pd-summary small,.pd-health-grid small,.pd-analysis-meta small{font-size:11px;color:var(--adm-muted)}.pd-summary .bad b{color:var(--adm-red)}.pd-provider-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.pd-provider{padding:13px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-surface);min-width:0}.pd-provider.has-error{border-color:color-mix(in srgb,var(--adm-red) 32%,var(--adm-border))}.pd-provider>header{display:grid;grid-template-columns:34px minmax(0,1fr) auto;gap:8px;align-items:center}.pd-logo{width:34px;height:34px;border-radius:10px;border:1px solid var(--adm-border);display:grid;place-items:center;font-weight:900}.pd-logo.vercel,.pd-deploy-mark.vercel{background:#111;color:white}.pd-logo.render,.pd-deploy-mark.render{color:#7c3aed;background:#7c3aed10;border-color:#7c3aed33}.pd-provider header small{font-size:11px;font-weight:900;color:var(--adm-muted);letter-spacing:.09em}.pd-provider h3{margin:2px 0 0;font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pd-pill{display:inline-flex;align-items:center;padding:4px 6px;border-radius:999px;border:1px solid var(--adm-border);font-size:11px;font-weight:850;white-space:nowrap}.pd-pill.ok{color:var(--adm-success);border-color:color-mix(in srgb,var(--adm-success) 20%,transparent);background:color-mix(in srgb,var(--adm-success) 4%,transparent)}.pd-pill.info{color:var(--adm-blue);border-color:color-mix(in srgb,var(--adm-blue) 20%,transparent);background:color-mix(in srgb,var(--adm-blue) 4%,transparent)}.pd-pill.bad{color:var(--adm-red);border-color:color-mix(in srgb,var(--adm-red) 20%,transparent);background:color-mix(in srgb,var(--adm-red) 4%,transparent)}.pd-pill.muted{color:var(--adm-muted)}.pd-provider dl{display:grid;grid-template-columns:72px minmax(0,1fr);gap:5px 8px;margin:12px 0}.pd-provider dt{font-size:11px;color:var(--adm-muted)}.pd-provider dd{font-size:12px;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pd-provider dd a{color:var(--adm-accent);text-decoration:none}.pd-last{display:grid;gap:2px;padding:9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2)}.pd-last small{font-size:11px;font-weight:900;letter-spacing:.1em;color:var(--adm-muted)}.pd-last b{font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pd-last span{font-size:11px;color:var(--adm-muted)}.pd-failure-hint{display:flex;align-items:flex-start;gap:7px;margin-top:8px;padding:8px 9px;border:1px solid color-mix(in srgb,var(--adm-red) 25%,var(--adm-border));border-radius:9px;background:color-mix(in srgb,var(--adm-red) 4%,var(--adm-surface));color:var(--adm-red)}.pd-failure-hint span{font-size:11px;line-height:1.4;color:var(--adm-muted)}.pd-provider footer{display:flex;gap:5px;flex-wrap:wrap;margin-top:9px}.pd-box,.pd-panel-health{padding:14px;border:1px solid var(--adm-border);border-radius:14px;background:var(--adm-surface)}.pd-box>header,.pd-panel-health>header{display:flex;justify-content:space-between;gap:10px;align-items:flex-end;margin-bottom:10px}.pd-box h2,.pd-panel-health h2{margin:3px 0 0;font-size:16px}.pd-box header>span,.pd-panel-health header>span{font-size:11px;color:var(--adm-muted)}.pd-panel-health header>span.ok{color:var(--adm-success)}.pd-panel-health header>span.warn{color:var(--adm-amber)}.pd-incident{display:grid;grid-template-columns:32px minmax(0,1fr) auto;gap:9px;align-items:center;padding:11px 12px;border:1px solid color-mix(in srgb,var(--adm-red) 28%,var(--adm-border));border-radius:12px;background:color-mix(in srgb,var(--adm-red) 4%,var(--adm-surface))}.pd-incident>div:nth-child(2){display:grid;gap:2px;min-width:0}.pd-incident b{font-size:12px;color:var(--adm-text)}.pd-incident span{font-size:11px;color:var(--adm-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pd-timeline{display:grid;gap:6px}.pd-timeline article{display:grid;grid-template-columns:30px minmax(0,1fr) auto auto;gap:8px;align-items:center;padding:8px 9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2)}.pd-timeline article.failed{border-color:color-mix(in srgb,var(--adm-red) 28%,var(--adm-border))}.pd-deploy-mark{width:28px;height:28px;border-radius:8px;border:1px solid var(--adm-border);display:grid;place-items:center;font-size:12px;font-weight:900}.pd-timeline article>div:nth-child(2){min-width:0;display:grid;gap:2px}.pd-timeline b{font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pd-timeline article>div:nth-child(2)>span{font-size:11px;color:var(--adm-muted)}.pd-timeline a{font-size:11px;color:var(--adm-accent);text-decoration:none}.pd-deploy-actions{display:flex;gap:4px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.pd-deploy-actions button{border:1px solid var(--adm-border);border-radius:7px;padding:5px 6px;background:var(--adm-surface);color:var(--adm-text);font-size:11px}.pd-copy{font-size:12px;color:var(--adm-muted);line-height:1.5}.pd-load-actions,.pd-log-shortcuts{display:flex;gap:7px;flex-wrap:wrap}.pd-log-shortcuts{margin:10px 0}.pd-alerts{display:grid;gap:6px}.pd-alerts article{display:grid;grid-template-columns:24px minmax(0,1fr);gap:8px;padding:9px;border:1px solid var(--adm-border);border-radius:9px;background:var(--adm-surface2)}.pd-alerts article>span{width:22px;height:22px;border-radius:7px;display:grid;place-items:center;font-weight:900}.pd-alerts .erro>span{color:var(--adm-red);background:color-mix(in srgb,var(--adm-red) 6%,transparent)}.pd-alerts .aviso>span{color:var(--adm-amber);background:color-mix(in srgb,var(--adm-amber) 6%,transparent)}.pd-alerts .info>span{color:var(--adm-blue);background:color-mix(in srgb,var(--adm-blue) 6%,transparent)}.pd-alerts article>div{display:grid;gap:2px}.pd-alerts b{font-size:12px}.pd-alerts small{font-size:11px;color:var(--adm-muted);line-height:1.4}.pd-clear{display:flex;gap:8px;align-items:center;padding:10px;border:1px solid color-mix(in srgb,var(--adm-success) 20%,transparent);border-radius:10px;background:color-mix(in srgb,var(--adm-success) 4%,transparent);color:var(--adm-success)}.pd-clear>div{display:grid;gap:2px}.pd-clear b{font-size:12px;color:var(--adm-text)}.pd-clear small{font-size:11px;color:var(--adm-muted)}.pd-analysis-meta{grid-template-columns:repeat(3,minmax(0,1fr));margin-top:9px}.pd-analysis-meta b{font-size:12px;overflow-wrap:anywhere}.pd-empty,.pd-log-empty{min-height:100px;display:grid;place-items:center;align-content:center;gap:7px;color:var(--adm-muted);font-size:12px;text-align:center}.pd-empty a{color:var(--adm-accent)}.pd-log-empty{margin-top:10px;border:1px dashed var(--adm-border);border-radius:12px;padding:18px}.pd-log-empty b{color:var(--adm-text)}.pd-log-empty span{font-size:11px;max-width:480px;line-height:1.45}.pd-drawer{margin-top:11px;border:1px solid var(--adm-border);border-radius:12px;overflow:hidden;background:var(--adm-surface)}.pd-drawer>header{display:flex;justify-content:space-between;gap:8px;align-items:center;padding:10px 11px;background:var(--adm-surface2);border-bottom:1px solid var(--adm-border)}.pd-drawer>header>div{display:grid;gap:2px}.pd-drawer header b{font-size:12px}.pd-drawer header small{font-size:11px;color:var(--adm-muted)}.pd-drawer header button{border:0;background:transparent;color:var(--adm-muted);font-size:17px}.pd-env-list{display:grid}.pd-env-editor-wrap{display:grid;gap:8px;padding:10px;border-top:1px solid var(--adm-border);background:var(--adm-surface2)}.pd-env-editor-title{display:flex;justify-content:space-between;align-items:center;gap:8px}.pd-env-editor-title b{font-size:12px}.pd-env-editor-title button{border:0;background:transparent;color:var(--adm-accent);font-size:11px}.pd-env-editor{display:grid;grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr);gap:6px}.pd-env-editor input{min-width:0;padding:7px 8px;border:1px solid var(--adm-border);border-radius:7px;background:var(--adm-bg);color:var(--adm-text);font-size:12px}.pd-env-save{display:flex;justify-content:flex-end;gap:6px}.pd-env-save button,.pd-env-row-actions button{border:1px solid var(--adm-border);border-radius:7px;padding:7px 8px;background:var(--adm-surface);color:var(--adm-text);font-size:11px}.pd-env-save .primary{background:var(--adm-accent);border-color:var(--adm-accent);color:#fff}.pd-env-save button:disabled,.pd-env-row-actions button:disabled{opacity:.5}.pd-env-options{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:11px;color:var(--adm-muted)}.pd-env-options label{display:flex;align-items:center;gap:3px}.pd-env-options .sensitive{margin-left:auto}.pd-env-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:9px;padding:8px 10px;border-top:1px solid var(--adm-border)}.pd-env-row:first-child{border-top:0}.pd-env-row>div{display:grid;gap:2px;min-width:0}.pd-env-row b{font-size:12px;overflow-wrap:anywhere}.pd-env-row small,.pd-env-row code{font-size:11px;color:var(--adm-muted);min-width:0}.pd-env-row code{max-width:min(42vw,360px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pd-env-row code.revealed{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;color:var(--adm-text)}.pd-env-limit{color:var(--adm-amber,#d97706)!important;overflow-wrap:anywhere}.pd-env-row-actions{display:flex;gap:4px}.pd-env-row-actions .danger{color:var(--adm-red)}.pd-log-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;padding:10px 11px;border-bottom:1px solid var(--adm-border);background:var(--adm-surface2)}.pd-log-head>div:first-child{display:grid;gap:2px}.pd-log-head small{font-size:11px;font-weight:900;color:var(--adm-accent);letter-spacing:.11em}.pd-log-head b{font-size:11px}.pd-log-head span{font-size:11px;color:var(--adm-muted)}.pd-log-tools{display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end}.pd-log-diagnosis{display:grid;grid-template-columns:24px minmax(0,1fr);gap:8px;margin:10px;padding:10px;border:1px solid color-mix(in srgb,var(--adm-red) 26%,var(--adm-border));border-radius:10px;background:color-mix(in srgb,var(--adm-red) 4%,var(--adm-surface))}.pd-log-diagnosis>span{width:22px;height:22px;border-radius:7px;display:grid;place-items:center;background:color-mix(in srgb,var(--adm-red) 8%,transparent);color:var(--adm-red);font-weight:900}.pd-log-diagnosis>div{display:grid;gap:3px;min-width:0}.pd-log-diagnosis b{font-size:12px;color:var(--adm-text)}.pd-log-diagnosis code{font:500 11px/1.45 monospace;color:var(--adm-red);white-space:pre-wrap;overflow-wrap:anywhere}.pd-log-list{display:grid;gap:5px;padding:9px;max-height:54vh;overflow:auto;background:var(--adm-bg)}.pd-log-row{display:grid;grid-template-columns:70px minmax(0,1fr);gap:8px;padding:8px 9px;border:1px solid var(--adm-border);border-radius:8px;background:var(--adm-surface)}.pd-log-row.bad{border-color:color-mix(in srgb,var(--adm-red) 28%,var(--adm-border));background:color-mix(in srgb,var(--adm-red) 3%,var(--adm-surface))}.pd-log-row.warn{border-color:color-mix(in srgb,var(--adm-amber) 24%,var(--adm-border))}.pd-log-row.ok{border-color:color-mix(in srgb,var(--adm-success) 20%,var(--adm-border))}.pd-log-meta{display:grid;align-content:start;gap:3px}.pd-log-meta span{font-size:11px;color:var(--adm-muted)}.pd-log-meta b{font-size:11px;text-transform:uppercase;color:var(--adm-accent)}.pd-log-row code{white-space:pre-wrap;overflow-wrap:anywhere;font:500 11px/1.5 monospace;color:var(--adm-text)}.pd-drawer-msg{padding:14px;font-size:12px;color:var(--adm-muted)}
        @media(max-width:760px){.pd-env-row{grid-template-columns:minmax(0,1fr)}.pd-env-row code{max-width:100%;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all}.pd-env-row-actions{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.pd-env-editor{grid-template-columns:1fr}.pd-env-options .sensitive{margin-left:0}.pd-env-save{display:grid;grid-template-columns:1fr 1fr}.pd-hero{display:grid}.pd-provider-grid{grid-template-columns:1fr}.pd-summary,.pd-health-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.pd-analysis-meta{grid-template-columns:1fr}.pd-timeline article{grid-template-columns:28px minmax(0,1fr) auto}.pd-deploy-actions{grid-column:2/-1;justify-content:flex-start}.pd-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.pd-actions>*{justify-content:center}.pd-incident{grid-template-columns:30px minmax(0,1fr)}.pd-incident button{grid-column:2}.pd-log-head{display:grid}.pd-log-tools{justify-content:flex-start}.pd-log-row{grid-template-columns:56px minmax(0,1fr)}}
      `}</style>
    </div>
  )
}

function EnvPanel({ env, onClose, onSave, onDelete, busy }) {
  ...
  const editing = Boolean(envId || (key && env.items?.some(x => x.key === key)))

  useEffect(() => { setRevealed({}) }, [env.items])
  useEffect(() => () => setRevealed({}), [])

  function reset() { setKey(''); setValue(''); setEnvId(''); setTarget(['production']); setSensitive(false) }
  function edit(item) {
    setKey(item.key || '')
    setEnvId(item.id || (env.kind === 'render' ? `render:${item.key}` : ''))
  ...
  function toggleTarget(name) {
    setTarget(old => old.includes(name) ? (old.length > 1 ? old.filter(x => x !== name) : old) : [...old, name])
  }
  async function submit(deploy = false) {
    const k = key.trim(), v = value
    if (!k || !v) return toast.error('Informe nome e valor da variável.')
    await onSave?.(k, v, deploy, { envId, target, sensitive })
    reset()
  }
  async function fetchValue(item) {
    if (item.revealable === false) throw new Error(item.providerLimitation || 'Este provedor não permite recuperar o valor original desta variável.')
    if (!env.resourceId) throw new Error('Recurso da plataforma não identificado.')
    const r = env.kind === 'vercel'
      ? await infraestruturaService.vercelRevelarVariavel(env.resourceId, item.key, item.id || '')
      : await infraestruturaService.renderRevelarVariavel(env.resourceId, item.key)
  ...
  async function copy(item) {
    ...
    setRevealBusy(rk)
    try {
      const v = revealed[rk] || await fetchValue(item)
  ...
  return (
    <div className="pd-drawer">
      <header><div><b>{env.title}</b><small>Mascaradas por padrão · valor completo somente sob demanda quando o provedor permitir</small></div><button onClick={onClose}>×</button></header>
      {env.loading ? <div className="pd-drawer-msg">Consultando variáveis…</div>
        : env.error ? <div className="pd-drawer-msg">{env.error}</div>
          : <>
            {env.items?.length ? <div className="pd-env-list">{env.items.map((x, i) => { const rk=x.id||x.key, rv=revealed[rk]; return <div className="pd-env-row" key={`${x.id || i}-${x.key}`}>
              <div><b>{x.key}</b><small>{env.kind === 'vercel' ? `${(x.target || []).join(', ') || 'produção'}${x.type ? ` · ${x.type}` : ''}` : 'serviço Render'} · {x.origin || env.kind} · {x.revealable === false ? 'protegida pelo provedor' : 'recuperável sob demanda'}{x.updatedAt ? ` · ${new Date(x.updatedAt).toLocaleString('pt-BR')}` : ''}</small>{x.providerLimitation&&<small className="pd-env-limit">{x.providerLimitation}</small>}</div>
              <code className={rv?'revealed':''}>{rv || x.valueMasked || 'protegida'}</code>
              <div className="pd-env-row-actions">{x.revealable!==false&&<><button disabled={busy||revealBusy===rk} onClick={() => reveal(x)}>{revealBusy===rk?'…':rv?'Ocultar':'Visualizar'}</button><button disabled={busy||revealBusy===rk} onClick={() => copy(x)}>Copiar</button></>}<button disabled={busy} onClick={() => edit(x)}>Editar</button><button className="danger" disabled={busy} onClick={() => onDelete?.(x)}>Excluir</button></div>
            </div>})}</div> : <div className="pd-drawer-msg">Nenhuma variável encontrada.</div>}
            {onSave && <div className="pd-env-editor-wrap">
              <div className="pd-env-editor-title"><b>{editing ? `Atualizar ${key}` : 'Adicionar variável'}</b>{editing && <button onClick={reset}>Cancelar edição</button>}</div>
              <div className="pd-env-editor">
                <input value={key} disabled={Boolean(envId)} onChange={e => setKey(e.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, ''))} placeholder="NOME_DA_VARIAVEL" />
                <input type="password" autoComplete="new-password" value={value} onChange={e => setValue(e.target.value)} placeholder={editing ? 'Digite o novo valor' : 'Valor'} />
              </div>
              {env.kind === 'vercel' && <div className="pd-env-options">
                <span>Ambientes:</span>{['production', 'preview', 'development'].map(t => <label key={t}><input type="checkbox" checked={target.includes(t)} onChange={() => toggleTarget(t)} /> {t}</label>)}
                <label className="sensitive"><input type="checkbox" checked={sensitive} onChange={e => setSensitive(e.target.checked)} /> Sensível</label>
              </div>}
              <div className="pd-env-save"><button disabled={busy || !key || !value} onClick={() => submit(false)}>Salvar</button><button className="primary" disabled={busy || !key || !value} onClick={() => submit(true)}>Salvar + deploy</button></div>
            </div>}
          </>}
    </div>
  )
