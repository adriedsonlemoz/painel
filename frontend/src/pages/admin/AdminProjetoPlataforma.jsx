...
function EnvPanel({ env, onClose, onSave, onDelete, busy }) {
  ...
  return (
    <div className="pd-drawer">
      <header><div><b>{env.title}</b><small>Segredos nunca são exibidos completos</small></div><button onClick={onClose}>×</button></header>
      {env.loading ? <div className="pd-drawer-msg">Consultando variáveis…</div>
        : env.error ? <div className="pd-drawer-msg">{env.error}</div>
          : <>
            {env.items?.length ? <div className="pd-env-list">{env.items.map((x, i) => <div className="pd-env-row" key={`${x.id || i}-${x.key}`}>
              <div><b>{x.key}</b><small>{env.kind === 'vercel' ? `${(x.target || []).join(', ') || 'produção'}${x.type ? ` · ${x.type}` : ''}` : 'serviço Render'}</small></div>
              <code>{x.valueMasked || 'protegida'}</code>
              <div className="pd-env-row-actions"><button disabled={busy} onClick={() => edit(x)}>Editar</button><button className="danger" disabled={busy} onClick={() => onDelete?.(x)}>Excluir</button></div>
