...
export default function AdminOnibus(){
  ...
  return <><ConfirmModal aberto={confirm.aberto} titulo="Excluir linha?" mensagem="Todos os horários desta linha serão removidos permanentemente." labelConfirmar="Excluir" carregando={confirm.carregando} onConfirmar={excluir} onCancelar={()=>setConfirm({aberto:false,id:null,carregando:false})}/>
    <DSPageHeader
      ...
      sub="Gerencie linhas, dados da viagem e horários publicados no portal."
      actions={editando===null ? <DSBtn variant="primary" onClick={()=>{setEditando('novo');setEditLinha(null)}}><Plus size={16}/>Nova linha</DSBtn> : null}
    />
    ...
    <>
      <DSStatGrid columns={3} mobileColumns={3} compact style={{ marginBottom:12 }}>
        <DSStatCard compact label="Linhas" value={linhas.length} />
        <DSStatCard compact label="Ativas" value={linhas.filter(l=>l.ativo).length} tone="success" />
        <DSStatCard compact label="Horários" value={totalHorarios} tone="info" />
      </DSStatGrid>
      <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:12,padding:'9px 10px',border:'1px solid var(--adm-border)',borderRadius:RADIUS.lg,background:'var(--adm-surface)'}}>
        <Search size={15} style={{color:'var(--adm-muted)',flexShrink:0}}/>
        <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar por destino, empresa ou código..." style={{flex:1,minWidth:0,border:0,outline:0,background:'transparent',color:'var(--adm-text)',font:'inherit'}}/>
        {busca && <button type="button" onClick={()=>setBusca('')} className="adm-btn adm-btn-ghost adm-btn-icon adm-btn-sm" title="Limpar busca"><X size={14}/></button>}
      </div>
    </>

    <div className="adm-card onibus-list-card" style={{padding:0,overflow:'hidden'}}>{loading?<div className="adm-empty"><p>Carregando...</p></div>:filtradas.length===0?<div className="adm-empty"><Bus size={32} style={{opacity:.2,marginBottom:SPACE.md}}/><p>{busca?'Nenhuma linha encontrada.':'Nenhuma linha cadastrada ainda.'}</p></div>:<div className="onibus-list">{filtradas.map(l=><div key={l.id} style={{background:'var(--adm-surface2)',border:'1px solid var(--adm-border)',borderRadius:RADIUS.lg,padding:16,marginBottom:SPACE.lg,opacity:l.ativo?1:.65}}>
      <div style={{display:'flex',alignItems:'center',gap:SPACE.lg,flexWrap:'wrap'}}><div style={{width:44,height:44,borderRadius:RADIUS.lg,backgroundColor:`${l.cor||'#1B5E3B'}20`,display:'flex',alignItems:'center',justifyContent:'center'}}><Bus size={20} style={{color:l.cor||'#1B5E3B'}}/></div><div style={{flex:1,minWidth:180}}><div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}><p style={{fontWeight:800,color:'var(--adm-text)'}}>{l.origem||'Iguatama'} <ArrowRight size={11} style={{display:'inline'}}/> {l.destino}</p>{l.codigo&&<span style={{fontSize:12,fontWeight:800,padding:'2px 6px',border:'1px solid var(--adm-border)',borderRadius:6}}>{l.codigo}</span>}<span style={{fontSize:12,fontWeight:800,color:l.ativo?'var(--adm-success,#22c55e)':'var(--adm-muted)'}}>{l.ativo?'ATIVA':'OCULTA'}</span></div><p style={{fontSize:FONT.base,color:'var(--adm-muted)',marginTop:3}}>{l.empresa||'Empresa não informada'} · {l.horarios?.length||0} horários{l.embarque?` · ${l.embarque}`:''}</p></div><div style={{display:'flex',gap:4}}><button onClick={()=>alternar(l)} className="adm-btn adm-btn-ghost adm-btn-icon adm-btn-sm" title={l.ativo?'Ocultar linha':'Ativar linha'}><Power size={15}/></button><button onClick={()=>duplicar(l)} className="adm-btn adm-btn-ghost adm-btn-icon adm-btn-sm" title="Duplicar"><Copy size={15}/></button><button onClick={()=>{setEditLinha(l);setEditando(l.id)}} className="adm-btn adm-btn-ghost adm-btn-icon adm-btn-sm" title="Editar"><Edit2 size={15}/></button><button onClick={()=>setConfirm({aberto:true,id:l.id,carregando:false})} className="adm-btn adm-btn-ghost adm-btn-icon adm-btn-sm" style={{color:'var(--adm-red)'}} title="Excluir"><Trash2 size={15}/></button></div></div>
    ...
    <style>{`
      .onibus-list{padding:14px}
      @media(max-width:640px){.onibus-list{padding:10px}.onibus-list>div{padding:12px!important}.onibus-list .adm-btn{min-width:32px}}
    `}</style>
  </>
}
