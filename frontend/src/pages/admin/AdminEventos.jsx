...
function EventoItem({ evento, passado, onEdit, onDelete }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'stretch', overflow:'hidden', marginBottom:10, border:'1px solid var(--adm-border)', borderRadius:RADIUS.xl, background:'var(--adm-surface2)', opacity:passado?.72:1 }}>
      <div style={{ width:66, flexShrink:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', borderRight:`3px solid ${cor}`, background:`${cor}12` }}><strong style={{ fontSize:22,lineHeight:1,color:'var(--adm-text)' }}>{data.getDate()}</strong><span style={{ marginTop:3,fontSize:12,fontWeight:800,textTransform:'uppercase',color:'var(--adm-muted)' }}>{data.toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</span></div>
